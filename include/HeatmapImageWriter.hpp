...
public:
  ...
  /**
   * @brief Fill a rectangle in a pixel buffer.
   */
  static void fillRect(std::vector<uint8_t> &pixels, int imgW, int imgH,
                       int x, int y, int w, int h, Color color);

  /**
   * @brief Draw a line in a pixel buffer (Bresenham).
   */
  static void drawLine(std::vector<uint8_t> &pixels, int imgW, int imgH,
...
inline HeatmapImageWriter::Color HeatmapImageWriter::valueToColor(float t) {
    // Clamp to [0,1]
    if (t < 0.0f) t = 0.0f;
    if (t > 1.0f) t = 1.0f;
...
inline void HeatmapImageWriter::drawChar(std::vector<uint8_t> &pixels, int imgW,
...
inline void HeatmapImageWriter::drawString(std::vector<uint8_t> &pixels, int imgW,
...
inline bool HeatmapImageWriter::writeRawPNG(const std::string &filename,
                                      const std::vector<uint8_t> &pixels,
                                      int width, int height) {
...
// ============================================================
// Percentile-based normalization helpers
// ============================================================

/**
 * Compute the normalization range using percentile clamping.
 * Collects off-diagonal values, sorts them, and returns the 2nd and 98th
 * percentile as (vmin, vmax) so outliers don't crush the color scale.
 * Uses log1p transform when data is non-negative and heavy-tailed
 * (98th/2nd percentile ratio > 100), which is typical for contact
 * frequency and sparse contact maps.
 */
static inline void computePercentileRange(const Heatmap &heat,
                                          float &vmin, float &vmax,
                                          bool &use_log, float &log_floor) {
    ...
    for (int i = 0; i < n; ++i)
        for (int j = 0; j < n; ++j)
            if (i != j) {
                vals.push_back(heat.v[i][j]);
                if (heat.v[i][j] < 0.0f) all_nonneg = false;
            }
    if (vals.size() < 10) {
        vmin = 1e30f; vmax = -1e30f;
        for (int i = 0; i < n; ++i)
            for (int j = 0; j < n; ++j) {
                if (heat.v[i][j] < vmin) vmin = heat.v[i][j];
                if (heat.v[i][j] > vmax) vmax = heat.v[i][j];
            }
        return;
    }
    // First pass: compute raw percentiles to detect heavy-tailed data
    std::sort(vals.begin(), vals.end());
    size_t lo = static_cast<size_t>(vals.size() * 0.02);
    size_t hi = static_cast<size_t>(vals.size() * 0.98);
    if (hi >= vals.size()) hi = vals.size() - 1;
    float raw_lo = vals[lo];
    float raw_hi = vals[hi];
    // Use log scale when data is non-negative and heavy-tailed.
    // Collect positive values, check if 98th/2nd percentile ratio > 10.
    if (all_nonneg && raw_hi > 0.0f) {
        // Gather positive (non-zero) values for ratio test
        std::vector<float> pos_vals;
        for (auto v : vals)
            if (v > 0.0f) pos_vals.push_back(v);
        if (pos_vals.size() > 10) {
            std::sort(pos_vals.begin(), pos_vals.end());
            float p02 = pos_vals[static_cast<size_t>(pos_vals.size() * 0.02)];
            float p98 = pos_vals[static_cast<size_t>(
                std::min(pos_vals.size() - 1, (size_t)(pos_vals.size() * 0.98)))];
            if (p02 > 0.0f && p98 / p02 > 10.0f) {
                use_log = true;
                log_floor = pos_vals.front();
                float floor_val = log_floor;
                for (auto &v : vals)
                    v = std::log(std::max(v, floor_val));
                std::sort(vals.begin(), vals.end());
                lo = static_cast<size_t>(vals.size() * 0.02);
                hi = static_cast<size_t>(vals.size() * 0.98);
                if (hi >= vals.size()) hi = vals.size() - 1;
            }
        }
    }
...
inline bool HeatmapImageWriter::writePNG(const Heatmap &heat,
    ...
    // Compute value range using percentile clamping
    float vmin, vmax;
    bool use_log = false;
    float log_floor = 1e-30f;
    computePercentileRange(heat, vmin, vmax, use_log, log_floor);
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            float val = use_log ? std::log(std::max(heat.v[i][j], log_floor)) : heat.v[i][j];
            float t = (val - vmin) / range;
            if (t < 0.0f) t = 0.0f;
            if (t > 1.0f) t = 1.0f;
...
inline bool HeatmapImageWriter::writeSVG(const Heatmap &heat,
                                   const std::string &filename,
                                   const std::string &title) {
    if (heat.size == 0) return false;
    ...
    // Compute range using percentile clamping
    float vmin, vmax;
    bool use_log = false;
    float log_floor = 1e-30f;
    computePercentileRange(heat, vmin, vmax, use_log, log_floor);
...
// ============================================================
// Generic pixel-buffer helpers (used by SvgChartGenerator PNG)
// ============================================================

inline void HeatmapImageWriter::fillRect(std::vector<uint8_t> &pixels, int imgW,
...
inline void HeatmapImageWriter::drawLine(std::vector<uint8_t> &pixels, int imgW,
                                   int imgH, int x0, int y0, int x1, int y1,
                                   Color color) {
