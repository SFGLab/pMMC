...
public:
  ...
  /**
   * @brief Load anchor positions directly from pre-parsed MergedAnchor vectors.
   * @param merged_anchors Pre-parsed anchors (from ibed/contact/merged formats).
   */
  void loadAnchorsFromMemory(const std::vector<MergedAnchor> &merged_anchors);

  /**
   * @brief Load arc data directly from pre-parsed MergedLoop vectors.
   * @param merged_loops Pre-parsed loops (from ibed/contact/merged formats).
   * @param factor_name Name of the factor (e.g. "contact", "ibed").
   * @param predefined_segments Segment boundaries for cross-segment filtering.
   */
  void loadArcsFromMemory(const std::vector<MergedLoop> &merged_loops,
...
inline InteractionArcs::InteractionArcs() {
  selected_region.chr = "";
  selected_region.start = 0;
  selected_region.end = 0; // this means that no region is selected (ie. we work
                           // on whole chromosomes)
}

inline void InteractionArcs::clear() {
  anchors.clear();
  arcs.clear();
  raw_arcs.clear();
...
inline void InteractionArcs::selectRegion(BedRegion region) {
...
inline void InteractionArcs::markArcs(bool ignore_missing) {
      ...
      if (i < cnt) {
        for (int j = 0; j < anchors_cnt[chr]; ++j) {
          if (anchors[chr][j].length() > 1) {
            if (anchors[chr][j].contains(raw_arcs[chr][i].start))
              st = j;
            if (anchors[chr][j].contains(raw_arcs[chr][i].end))
              end = j;
          }
        }
...
inline void InteractionArcs::removeEmptyAnchors() {
  ...
  for (auto el : anchors) {
    ...
    for (Anchor a : correct_anchors)
      this->anchors[el.first].push_back(a);

    int removed_cnt = anchors_cnt[el.first] - static_cast<int>(anchors[el.first].size());
    anchors_cnt[el.first] -= removed_cnt; // update number of anchors
    printf("   %s %d\n", el.first.c_str(), removed_cnt);
  }
}

inline void InteractionArcs::rewire() {
  // for each anchor find the total strength of anchored interactions (ie. the
  // sum of PET counts of arcs ending at the anchor) probabilistically remove
  // anchors based on this strength

  // printf("rewire!\n");
  for (auto el : anchors) { // iterate over chromosomes
    for (int i = anchors_cnt[el.first] - 1; i >= 0;
         i--) { // iterate over anchors
      // printf("anchor %d\n", i);
      ...
      for (int j = 0; j < arcs_cnt[el.first]; j++) {
        if (arcs[el.first][j].start == i || arcs[el.first][j].end == i) {
          ...
          // printf("   arc %d %d   %d %d\n", arcs[el.first][j].start,
          // arcs[el.first][j].end, arcs[el.first][j].eff_score,
          // arcs[el.first][j].score);
        }
      }
      // printf("total: %d\n", sum);
    ...
    anchors_cnt[el.first] = static_cast<int>(anchors[el.first].size());
  }
}
...
//		for (int j=0, p=anchors[i-1].center+d; j<cnt && p <
//anchors[i].center; j++, p+=d) {
//			//printf("%d (%d %d) %d\n", i, anchors[i-1].center,
//anchors[i].center, d); 			Anchor tmp("", p, p); 			new_anchors.push_back(tmp);
//		}
//	}
...
//		printf("len=%d, dist=%d\n", anchors[i].length(),
//anchors[i].center-anchors[i-1].center);
//	}
//
//	//for (int i = 1; i < points.size() && anchor_start.size(); ++i) {
//	//	printf("len=%d, dist=%d\n", anchor_end[i]-anchor_start[i],
//anchor_start[i]-anchor_end[i-1]);
//	//}
// }

// note: anchor file must be sorted
inline void InteractionArcs::loadAnchorsData(string anchors_path) {
  ...
  // recognize format. allowed formats are "chr start end" and "chr start end
  // orientation"
  ...
  if (fgets(line, 64, f) != NULL) {
    ...
    if (cnt != 3 && cnt != 4)
      error("unrecognized format of anchor file");
    if (cnt == 4) {
      motif_info = true;
  ...
  // REQ-3.4: read anchor file line by line using fgets+sscanf (buffered)
  char anchor_line[256];
  while (fgets(anchor_line, sizeof(anchor_line), f) != NULL) {

    int cols = 0; // number of values (columns) read from the line
    if (motif_info)
      cols = sscanf(anchor_line, "%s %d %d %c", chr_tmp, &posa, &posb, &orientation);
    else
      cols = sscanf(anchor_line, "%s %d %d", chr_tmp, &posa, &posb);

    if (cols < 3)
      continue; // probably just an empty line at the end

    chr = string(chr_tmp);

    // check if chromosome is ok
    // then check if the anchor fits into the selected region (at least one end
    // must be within)
    if (chrs_set.find(chr) == chrs_set.end())
      continue;
    if (is_region_selected &&
        (!selected_region.contains(posa) && !selected_region.contains(posb)))
      continue;
  ...
  for (auto el : anchors) {
    anchors_cnt[el.first] = static_cast<int>(el.second.size());
    printf("   %s: %d\n", el.first.c_str(), anchors_cnt[el.first]);
  }
}

// Reads clusters from file
// predefined_segments - bed regions (usually defining segments); clusters
// overlapping their ends will be ignored. Used to ensure there will be gaps
// where custom split is
inline void InteractionArcs::loadPetClustersData(string pet_clusters_path,
                                          string factor_name,
                                          BedRegions predefined_segments) {
  printf("read clusters [%s] (factor = %s)\n", pet_clusters_path.c_str(),
         factor_name.c_str());
  ...
  int arcs_over_splits = 0; // how many arcs were dropped because they stretched
                            // over the custom split sites

  int factor = static_cast<int>(factors.size());
  ...
  // REQ-7.1: estimate file size for reserve()
  fseek(f, 0, SEEK_END);
  long file_size = ftell(f);
  fseek(f, 0, SEEK_SET);
  if (file_size > 0) {
    // Estimate ~80 bytes per line in BEDPE format
    size_t estimated_arcs = (size_t)(file_size / 80);
    for (std::string chr : chrs) {
      // Reserve per-chr capacity (rough: divide by expected number of chrs)
      size_t per_chr = estimated_arcs / std::max((size_t)1, chrs.size());
      if (raw_arcs[chr].capacity() < per_chr)
        raw_arcs[chr].reserve(per_chr);
    }
  }

  char line_buf[2048]; // REQ-3.3: line-buffered reading
  for (int i = 1; !feof(f); ++i) {
    if (i % 1000000 == 0) {
      printf(".");
      // REQ-2.2: periodic memory logging
      if (i % 5000000 == 0)
        logMemoryUsage(ftext("loadPetClusters %dk arcs", i / 1000).c_str());
    }
    // REQ-5.1: cancellation check
    if (g_cancel_requested.load(std::memory_order_relaxed)) {
      printf("\n[CANCEL] loadPetClustersData interrupted\n");
      break;
    }

    // REQ-3.3: buffered line read then parse
    if (fgets(line_buf, sizeof(line_buf), f) == NULL)
      break;
    sc = 1.0f; // default score for BEDPE without score column
    int nfields = sscanf(line_buf, "%s %d %d %s %d %d %f", chr_a, &ast, &aend, chr_b, &bst,
               &bend, &sc);
    if (nfields < 6) // accept 6-col BEDPE (no score) or 7-col (with score)
      continue;

    if (strcmp(chr_a, chr_b) != 0)
      continue; // only intra contacts

    // check if the arc belongs to the selected chromosomes
    if (chrs_set.find(chr_a) == chrs_set.end())
      continue;
    ...
    InteractionArc arc(posa, posb, static_cast<int>(sc), factor);
    ...
    // REQ-3.1: append to end (O(1)) instead of insert-in-middle (O(N)).
    // Sorting will be done once after the loop completes.
    raw_arcs[chr_a].push_back(arc);
    added++;
  }
  fclose(f);

  // REQ-3.1: sort raw_arcs by start position after loading (O(N log N) total).
  // This replaces the previous O(N²) insert-in-sorted-position pattern.
  for (std::string chr : chrs) {
    std::sort(raw_arcs[chr].begin(), raw_arcs[chr].end());
  }

  printf("added %d, discarded long arcs: %d, over custom split: %d\n", added,
         long_arcs_cnt, arcs_over_splits);
}

// ---------------------------------------------------------------------------
// Load anchors directly from pre-parsed MergedAnchor vectors (no file I/O)
// ---------------------------------------------------------------------------
inline void InteractionArcs::loadAnchorsFromMemory(
    const std::vector<MergedAnchor> &merged_anchors) {

  printf("load anchors from memory (%d anchors)\n", (int)merged_anchors.size());

  std::set<std::string> chrs_set(chrs.begin(), chrs.end());
  bool is_region_selected = selected_region.end > 0;

  for (const auto &ma : merged_anchors) {
    if (chrs_set.find(ma.chr) == chrs_set.end())
      continue;
    if (is_region_selected &&
        !selected_region.contains(ma.start) && !selected_region.contains(ma.end))
      continue;

    Anchor anchor(ma.chr, ma.start, ma.end, 'N');
    anchors[ma.chr].push_back(anchor);
  }
...
// ---------------------------------------------------------------------------
// Load arcs directly from pre-parsed MergedLoop vectors (no file I/O)
// ---------------------------------------------------------------------------
inline void InteractionArcs::loadArcsFromMemory(
    const std::vector<MergedLoop> &merged_loops,
    const std::string &factor_name,
    BedRegions predefined_segments) {

  printf("load arcs from memory (%d loops, factor=%s)\n",
         (int)merged_loops.size(), factor_name.c_str());
  ...
  for (const std::string &chr : chrs) {
    if (raw_arcs[chr].capacity() < merged_loops.size() / std::max((size_t)1, chrs.size()))
      raw_arcs[chr].reserve(merged_loops.size() / std::max((size_t)1, chrs.size()));
  }

  int added = 0, long_arcs_cnt = 0, arcs_over_splits = 0;

  for (const auto &ml : merged_loops) {
    if (ml.chr1 != ml.chr2)
      continue; // intra-chromosomal only
    if (chrs_set.find(ml.chr1) == chrs_set.end())
      continue;

    int posa = (ml.start1 + ml.end1) / 2;
    int posb = (ml.start2 + ml.end2) / 2;
    ...
    raw_arcs[ml.chr1].push_back(arc);
    added++;
  }

  // Sort raw_arcs by start position
  for (const std::string &chr : chrs)
    std::sort(raw_arcs[chr].begin(), raw_arcs[chr].end());
...
inline void InteractionArcs::toFile(string filename) {
  ...
  for (string factor : factors)
    fprintf(f, "%s ", factor.c_str());
  fprintf(f, "\n");

  for (string chr : chrs) {
    fprintf(f, "%s %d %d %d\n", chr.c_str(), anchors_cnt[chr], arcs_cnt[chr],
            (int)long_arcs[chr].size());
    for (i = 0; i < anchors_cnt[chr]; ++i) {
      fprintf(f, "%d %d %c\n", anchors[chr][i].start, anchors[chr][i].end,
              anchors[chr][i].orientation);
    }

    for (int i = 0; i < arcs_cnt[chr]; ++i) {
      arc = &arcs[chr][i];
      fprintf(f, "%d %d %d %d %d %d %d\n", arc->start, arc->end,
              arc->genomic_start, arc->genomic_end, arc->score, arc->eff_score,
              arc->factor);
    }
...
inline bool InteractionArcs::fromFile(string filename) {
  ...
  if (fscanf(f, "%d %d", &cnt, &cnt_factors) != 2)
    return false;
  ...
  for (i = 0; i < cnt; ++i) {
    ...
    chr = string(chr_tmp);
    printf("[%s] %d %d %d\n", chr.c_str(), cnt_anchors, cnt_arcs,
           long_arcs_cnt);
    ...
    for (j = 0; j < cnt_arcs; ++j) {
      fscanf(f, "%d %d %d %d %d %d %d", &st, &end, &st_g, &end_g, &sc, &eff_sc,
             &fact);
...
inline void InteractionArcs::print(int display_limit) {
  ...
  for (auto el : anchors) {
    ...
    printf(" [%s] anchors: %d, arcs: %d\n", el.first.c_str(),
           anchors_cnt[el.first], arcs_cnt[el.first]);
    ...
    for (i = 0; i < display; ++i) {
      ...
      printf("   [%d] (%d %d), sc=%d (%d), f=%d (%d %d)\n", i, arc->start,
             arc->end, arc->score, arc->eff_score, arc->factor,
             arc->genomic_start, arc->genomic_end);
    }

    //		printf(" [%s] raw arcs %d\n", el.first.c_str(),
    //raw_arcs[el.first].size()); 		display = min((int)raw_arcs[el.first].size(),
    //display_limit); 		for (i = 0; i < display; ++i) { 			arc =
    //&raw_arcs[el.first][i]; 			printf("   [%d] (%d %d), sc=%d, f=%d)\n", i,
    //arc->start, arc->end, arc->score, arc->factor);
    //		}
  }
}
