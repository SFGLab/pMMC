...
#include <SphericalContainer.hpp>
#include <platform.h>
...
public:
  ...
  /** @brief Destructor: frees owned resources (e.g. spherical container). */
  ~LooperSolver();
  ...
  /**
   * @brief Set contact data directly from pre-parsed vectors (no file I/O).
   * Used by ibed/contact/merged input formats.
   */
  void setContactDataFromMemory(std::vector<string> chrs_list,
                                BedRegion region_of_interest,
                                const std::vector<MergedAnchor> &merged_anchors,
                                const std::vector<MergedLoop> &merged_loops,
  ...
  /**
   * @brief Store the current structure as an RMSD checkpoint (F5.3).
   *
   * Saves the 3D positions of all clusters in the active region so that
   * RMSD relative to this snapshot can be computed later.
   *
   * @param label A label for the checkpoint (e.g. "heatmap_initial").
   */
  void logRMSDCheckpoint(const std::string &label);

  /**
   * @brief Compute RMSD between the current structure and a stored checkpoint.
   * @param label Checkpoint label to compare against.
   * @return RMSD value, or -1.0 if the checkpoint was not found.
   */
  double computeRMSDvsCheckpoint(const std::string &label) const;

  /**
   * @brief Write accumulated RMSD convergence data to CSV.
   * @param suffix Filename suffix (e.g. "heatmap", "arcs").
   */
  void writeRMSDConvergenceCSV(const std::string &suffix) const;
...
private:
  ...
  SphericalContainer *spherical_container = nullptr;  /**< Optional spherical boundary (owned, deleted in destructor). */
  ...
  /**
   * @brief Load BED annotation files and mark matching anchors with annotation types.
   *
   * Reads enhancer, promoter, and epigenetic BED files (if configured) and
   * sets the annotation_type field on clusters whose genomic intervals overlap.
   */
  ...
  /** @brief Per-chromosome annotation types for anchors, indexed by anchor index.
   *  Populated by loadAnnotations(), applied to clusters during createTreeGenome(). */
  std::map<std::string, std::vector<AnnotationType>> anchor_annotations;
  ...
  /** @name RMSD Convergence Tracking (F5.3) */
  ///@{
  /** @brief Stored RMSD checkpoint positions, keyed by label. */
  std::map<std::string, std::vector<vector3>> rmsd_checkpoints;

  /** @brief RMSD convergence log entries: (label, rmsd_value). */
  std::vector<std::pair<std::string, double>> rmsd_convergence_log;
  ///@}
};
...
#include <array>
#include <chrono>
#include <MdsInitializer.hpp>
#include <MultiscaleEnergy.hpp>
#include <SvgChartGenerator.hpp>
#include <platform.h>
#include <rmsd.h>

inline LooperSolver::LooperSolver(string label, string outdir) {
  ...
  for (int i = 2; i <= 100; ++i)
    freq_to_distance[i] = freqToDistance(i, false);
  ...
  if (Settings::outputLevel >= 7) {
    ...
    float freq[] = {2.0f,  1.5f, 1.0f,  0.75f, 0.5f,
                    0.25f, 0.1f, 0.05f, 0.01f, 0.001f};
    for (int i = 0; i < 10; i++)
  ...
  if (Settings::dataSegmentsSplit.size()) {
    ...
    error("You need to provide the segment split");

  // Initialize optional spherical boundary
  if (Settings::sphere_radius > 0.0f) {
    vector3 origin(0.0f, 0.0f, 0.0f);
    spherical_container = new SphericalContainer(origin, Settings::sphere_radius);
    printf("spherical boundary enabled: radius = %f\n", Settings::sphere_radius);
  }
}

inline LooperSolver::~LooperSolver() {
  delete spherical_container;
}

inline void LooperSolver::print() {
  printf("*** clusters: %d\n", (int)clusters.size());
  printf("clusters\n");
...
inline void LooperSolver::initDensityData() {
  printf("init density\n");
  density.fromSegmentationFile(Settings::densityMapFile);
  density.normalize();
  density.toFile(ftext("%sdensity_segm.txt", output_dir.c_str()));

  density_curr.init(density.size_x, density.size_y, density.size_z,
                    density.range_x_start, density.range_y_start,
                    density.range_z_start, false);
}

inline void LooperSolver::printActiveRegion() {
  printf("*** active region: size = %d\n", (int)active_region.size());
  for (size_t i = 0; i < active_region.size(); ++i) {
    printf("[%zu - %d] ", i, active_region[i]);
    if (i + 1 < active_region.size()) {
      printf(
          "(dist exp=%lf, true=%lf) ", clusters[active_region[i]].dist_to_next,
          (clusters[active_region[i]].pos - clusters[active_region[i + 1]].pos)
              .length());
    }
...
inline void LooperSolver::reconstructClustersHeatmap() {

  logMemoryUsage("reconstructClustersHeatmap start");
  heatmap.clear();

  // RMSD convergence tracking (F5.3): save initial checkpoint
  logRMSDCheckpoint("heatmap_initial");
  ...
  if (Settings::randomWalk) {
    ...
    float size = static_cast<float>(genomicLengthToDistance(1000000));
    for (string chr : chrs) {
      printf("%s\n", chr.c_str());
      rw_pos.set(0.0f, 0.0f, 0.0f);
    ...
    // if entire region is contained in a single segment then there is no
    // heatmap we can just position the corresponding bead in (0,0,0) and go to
    // the anchor level
    ...
    bool chromosome_level =
        chrs.size() > 1; // do we need to create a heatmap on chromosome level?
    ...
    if (chromosome_level) {
      ...
      // heat_file = ftext("%ssingletons_%s_chromosomes.heat",
      // output_dir.c_str(), label.c_str());
      heat_file = ftext("%ssingletons_chromosomes_%s.heat", output_dir.c_str(),
                        label.c_str());
      Heatmap hc, hnc;
      ...
      // hnc.toFile(ftext("%ssingletons_%s_chromosomes_norm.heat",
      // output_dir.c_str(), label.c_str()));
      hnc.toFile(ftext("%ssingletons_chromosomes_norm_%s.heat",
                       output_dir.c_str(), label.c_str()));

      heatmap_chromosome_boundaries
          .clear(); // make sure it is empty for chr level (MC calc score)

      // note that order of pushing heatmaps is important,
      // reconstructClustersHeatmapSingleLevel() take index of heatmap as a
      // parameter
      heatmap.push_back(hnc);

      createDistanceHeatmap(0);
      heatmap_dist.toFile(ftext("%sheat_dist_chromosomes_%s.heat",
                                output_dir.c_str(), label.c_str()),
                          true);
      ...
      // even if we don't have chromosome level we need to put a heatmap for we
      // want the indices of levels and heatmaps to be equal
      Heatmap tmp;
    ...
    // the structure on the segment level can be obtained from the template
    // structure it can be either .hcm model or .chr/.txt file with coordinates
    if (Settings::templateSegment.size() > 1) {
      ...
      if (Settings::templateSegment.find(".hcm") != std::string::npos) {
        ...
        tpl.fromFile(Settings::templateSegment.c_str());
      ...
      if (Settings::distHeatmap.size() > 0) {
        ...
        int tot_size = 0;
        for (string chr : chrs)
          tot_size += static_cast<int>(current_level[chr].size());
        ...
        // label.c_str());
        heat_file = ftext("%ssingletons_segment_%s.heat", output_dir.c_str(),
                          label.c_str());
        ...
        // hn.toFile(ftext("%ssingletons_%s_segment_norm.heat",
        // output_dir.c_str(), label.c_str()));
        hn.toFile(ftext("%ssingletons_segment_norm_%s.heat", output_dir.c_str(),
                        label.c_str()));
        heatmap.push_back(hn);
        createDistanceHeatmap(1);
        heatmap_dist.toFile(ftext("%sheat_dist_segment_%s.heat",
                                  output_dir.c_str(), label.c_str()),
                            true);
      }
      ...
      if (chrs.size() > 1) {
        ...
        for (string chr : chrs) {
          p += static_cast<int>(current_level[chr].size());
          heatmap_chromosome_boundaries.push_back(p);
        }
        printv(heatmap_chromosome_boundaries, true, true,
  ...
  // RMSD convergence tracking (F5.3): compute RMSD vs initial after heatmap phase
  {
    double rmsd = computeRMSDvsCheckpoint("heatmap_initial");
    if (rmsd >= 0.0) {
      rmsd_convergence_log.push_back({"heatmap_final", rmsd});
      output(1, "[RMSD] Heatmap phase: RMSD vs initial = %.4f\n", rmsd);
    }
    writeRMSDConvergenceCSV(label);
  }
}

// level - which heatmap to use, 0 is the most general
inline void LooperSolver::reconstructClustersHeatmapSingleLevel(int level) {
  setLevel(level); // set the corresponding level (here we index from 0, and
                   // levels are 1-based)

  // set initial structure and average distance between neighboring beads
  float avg_dist = 0.0f;
  std::vector<vector3>
      initial_structure; // store initial position for all regions
  if (level == LVL_CHROMOSOME)
    avg_dist =
        heatmap_dist.getAvg() *
        Settings::noiseCoefficientLevelChr; // position randomly around the
                                            // origin (a point for chromosome)
  if (level == LVL_SEGMENT) {
    ...
    for (string chr : chrs) {
      ...
      if (Settings::useDensity)
        origin = density.center;
      if (Settings::useTelomerePositions)
        origin =
            (Settings::telomere_1_position + Settings::telomere_2_position) /
            2.0f;
  ...
  // for every el of active_region we need to know which chromosome it belongs
  // to, so we keep track of how many segments there are for every chromosome
  // (see activeRegionIndexToChr())
  active_region.clear();
  active_region_to_chr.clear();
  for (string chr : chrs) {
    active_region.insert(active_region.end(), current_level[chr].begin(),
                         current_level[chr].end());
    active_region_to_chr[chr] = static_cast<int>(active_region.size());
  }
  ...
  for (size_t i = 0; i < active_region.size(); ++i)
    initial_structure.push_back(clusters[active_region[i]].pos);

  int steps = level == LVL_CHROMOSOME ? Settings::simulationStepsLevelChr
                                      : Settings::simulationStepsLevelSegment;

  double score, best_score = -1.0;
  std::vector<vector3> best_structure; // to save best structure
  // MDS initialization (R-G6): use distance matrix to compute initial 3D positions
  bool mds_initialized = false;
  std::vector<vector3> mds_positions;
  if (Settings::initMethod == "mds" && heatmap_dist.size > 0) {
    printf("[MDS] Computing initial positions from distance heatmap (%zd beads)...\n",
           heatmap_dist.size);
    mds_positions = MdsInitializer::computeCoordinates(heatmap_dist);
    if (!mds_positions.empty() && mds_positions.size() == active_region.size()) {
      mds_initialized = true;
      printf("[MDS] Initialization successful (%zu positions)\n", mds_positions.size());
    } else {
      printf("[MDS] Initialization failed or size mismatch (%zu vs %zu), falling back to random\n",
             mds_positions.size(), active_region.size());
    }
  }

  for (int k = 0; k < steps; ++k) {

    // Set initial positions: MDS (first iteration) or random perturbation
    if (mds_initialized && k == 0) {
      // Use MDS positions for the first trial
      for (size_t i = 0; i < active_region.size(); ++i) {
        clusters[active_region[i]].pos =
            mds_positions[i] + random_vector(avg_dist * 0.1f, Settings::use2D);
      }
    } else {
      // Random initial position (use initial_structure and add some noise)
      for (size_t i = 0; i < active_region.size(); ++i) {
        clusters[active_region[i]].pos =
            initial_structure[i] + random_vector(avg_dist, Settings::use2D);
      }
    }
    ...
    if (level == LVL_SEGMENT && Settings::useTelomerePositions) {
      ...
      //			print_vector(Settings::telomere_1_position, "tel
      //1 org");
      //			print_vector(densityCoordToStructure(Settings::telomere_1_position),
      //"tel 1 tr");
      //
      //			print_vector(Settings::telomere_2_position, "tel
      //2 org");
      //			print_vector(densityCoordToStructure(Settings::telomere_2_position),
      //"tel 2 tr");

      clusters[active_region[0]].pos = densityCoordToStructure(
          Settings::telomere_1_position - density.origin);
      clusters[active_region[0]].is_fixed = true;
      int last = static_cast<int>(active_region.size()) - 1;
      clusters[active_region[last]].pos = densityCoordToStructure(
          Settings::telomere_2_position - density.origin);
    ...
    // Energy trace logging after GPU heatmap kernel
    if (Settings::energyTraceEnabled && sim_logger) {
      sim_logger->logEnergy(total_mc_steps, 0.0,
                            score, 0.0, score, 0.0, 0.0);
    }
    ...
    if (score < best_score || best_score < 0.0) {
      ...
      for (size_t i = 0; i < active_region.size(); ++i)
        best_structure.push_back(clusters[active_region[i]].pos);
      best_score = score;
    }
  }
...
inline double LooperSolver::MonteCarloHeatmap(float step_size) {
  ...
  double milestone_score; // score measured every N steps, used to see whether
                          // the solution significantly improves
  int success = 0;        // overall number of successes
  int milestone_success =
      0;      // calc how many successes there were since last milestone
  string chr; // tmp variable to keep track to which chromosome a specific point
              // belongs
  int size = static_cast<int>(active_region.size());

  if (size <= 1)
    return 0.0; // there is nothing to do

  // calc initial score
  score_curr = calcScoreHeatmapActiveRegion(); // score from heatmap
  score_density = calcScoreDensity();

  // Compute initial boundary penalty sum if spherical container is active
  double boundary_sum = 0.0;
  if (spherical_container) {
    for (int k = 0; k < size; k++)
      boundary_sum += spherical_container->boundaryPenalty(clusters[active_region[k]].pos);
    score_curr += boundary_sum;
  }
  ...
  while (true) {
    ...
    if (clusters[ind].is_fixed)
      continue; // check if bead is fixed (for example telomere)

    // Track boundary penalty change for the moved bead
    double old_bead_penalty = 0.0;
    if (spherical_container)
      old_bead_penalty = spherical_container->boundaryPenalty(clusters[ind].pos);

    displacement = random_vector(
        step_size, Settings::use2D); // generate random displacement vector
    clusters[ind].pos += displacement;
    score_curr = calcScoreHeatmapActiveRegion();

    // Update boundary sum incrementally: subtract old, add new
    if (spherical_container) {
      double new_bead_penalty = spherical_container->boundaryPenalty(clusters[ind].pos);
      score_curr += boundary_sum - old_bead_penalty + new_bead_penalty;
    }

    ok = score_curr <= score_prev;

    if (!ok && T > 0.0) {
      tp = Settings::tempJumpScaleHeatmap *
           exp(-Settings::tempJumpCoefHeatmap * (score_curr / score_prev) / T);
      ok = withChance(static_cast<float>(tp));
    ...
    if (ok) {
      success++;
      milestone_success++;
      // Update running boundary sum on acceptance
      if (spherical_container) {
        double new_bead_penalty = spherical_container->boundaryPenalty(clusters[ind].pos);
        boundary_sum = boundary_sum - old_bead_penalty + new_bead_penalty;
      }
    } else {
      clusters[ind].pos -= displacement;
    ...
    // Energy trace logging (R-G9)
    if (Settings::energyTraceEnabled && sim_logger &&
        i % Settings::energyTraceInterval == 0) {
      double accept_ratio = (i > 0) ? (double)success / i : 0.0;
      sim_logger->logEnergy(total_mc_steps + i, T,
                            score_curr, 0.0, score_curr,
                            accept_ratio, 0.0);
    }
    ...
    if (i % Settings::MCstopConditionStepsHeatmap == 0) {
      ...
      output(2,
             "milestone [%d]: score = %lf, last = %lf (%f), T=%lf successes = "
             "%d, last = %d  (den=%lf), step=%f\n",
             milestone_cnt, score_curr, milestone_score,
             score_curr / milestone_score, T, success, milestone_success,
             score_density, step_size);

      // stop if the improvement since the last milestone is less than 0.5%, or
      // if the score is too small (may happen when heatmap is small and points
      // are perfectly arranged)
      if ((score_curr >
               Settings::MCstopConditionImprovementHeatmap * milestone_score &&
           milestone_success < Settings::MCstopConditionMinSuccessesHeatmap) ||
          score_curr < 1e-6) {
        break;
      }
      milestone_score = score_curr;
  ...
  // Log final acceptance ratio for this phase
  if (sim_logger)
    sim_logger->logAcceptanceRatio("heatmap_cpu", i, success);

  total_mc_steps += i;
  return score_curr;
}

inline double LooperSolver::MonteCarloHeatmapAndDensity(float step_size) {
  ...
  // Compute initial boundary penalty sum if spherical container is active
  double boundary_sum_hd = 0.0;
  if (spherical_container) {
    for (int k = 0; k < size; k++)
      boundary_sum_hd += spherical_container->boundaryPenalty(clusters[active_region[k]].pos);
    score_curr += boundary_sum_hd;
  }
  ...
  while (true) {
    ...
    // Track boundary penalty for the moved bead before move
    double old_bead_penalty_hd = 0.0;
    if (spherical_container)
      old_bead_penalty_hd = spherical_container->boundaryPenalty(clusters[ind].pos);
    ...
    // Update boundary sum incrementally
    if (spherical_container) {
      double new_bead_penalty_hd = spherical_container->boundaryPenalty(clusters[ind].pos);
      score_curr += boundary_sum_hd - old_bead_penalty_hd + new_bead_penalty_hd;
    }

    ok = score_curr <= score_prev;

    if (!ok && T > 0.0) {
      tp = Settings::tempJumpScaleHeatmapDensity *
           exp(-Settings::tempJumpCoefHeatmapDensity *
               (score_curr / score_prev) / T);
      // printf("!!!");
      ok = withChance(static_cast<float>(tp));
    }
    ...
    if (ok) {
      ...
      if (spherical_container) {
        double new_bead_penalty_hd = spherical_container->boundaryPenalty(clusters[ind].pos);
        boundary_sum_hd = boundary_sum_hd - old_bead_penalty_hd + new_bead_penalty_hd;
      }

      // output(2, "ok: %lf (heat=%lf, density=%lf)   %f\n", score_curr,
      // score_heatmap, score_density, T); if (success > 10) break;
    } else {
      clusters[ind].pos -= displacement;
      score_curr = score_prev; // score doesn't change
    ...
    if (i % Settings::MCstopConditionStepsHeatmapDensity == 0) {
      ...
      output(2,
             "milestone [%d]: score = %lf, last = %lf (%f), T=%lf successes = "
             "%d, last = %d  (dens=%lf, heat=%lf), step=%f\n",
             milestone_cnt, score_curr, milestone_score,
             score_curr / milestone_score, T, success, milestone_success,
             score_density, score_heatmap, step_size);

      // stop if the improvement since the last milestone is less than 0.5%, or
      // if the score is too small (may happen when heatmap is small and points
      // are perfectly arranged)
      if ((score_curr > Settings::MCstopConditionImprovementHeatmapDensity *
                            milestone_score &&
           milestone_success <
               Settings::MCstopConditionMinSuccessesHeatmapDensity) ||
          score_curr < 1e-6) {
        break;
      }
      milestone_score = score_curr;
...
inline void LooperSolver::setContactData(std::vector<string> chrs_list,
                                  BedRegion region_of_interest, string anchors,
                                  std::vector<string> factors,
                                  std::vector<string> arcs_clusters,
                                  std::vector<string> arcs_singletons,
  ...
  if (factors.size() != arcs_clusters.size())
    error("Number of clusters is different then number of factors (see "
          "[data]->[clusters] and [factors])");
  ...
  if (Settings::useInputCache) {
    ...
    if (file_exists(input_file)) {
      ...
      if (arcs.fromFile(input_file))
        data_loaded = true;
      else
        printf(" problem with cache file, recreate it...\n");
    }
  } else {
  ...
  if (!data_loaded) {
    ...
    for (uint i = 0; i < factors.size(); ++i)
      arcs.loadPetClustersData(arcs_clusters[i], factors[i],
                               segments_predefined);
    ...
    // REQ-3.5: free raw_arcs after markArcs — no longer needed
    for (auto &kv : arcs.raw_arcs) {
      kv.second.clear();
      kv.second.shrink_to_fit();
    }
    arcs.raw_arcs.clear();
    logMemoryUsage("after raw_arcs freed");

    if (Settings::useInputCache)
      arcs.toFile(input_file);
  }

  logMemoryUsage("after setContactData");

  // REQ-4.1: check memory budget after data load
  if (checkMemoryBudget(Settings::maxMemoryMB, "setContactData")) {
    printf("[ERROR] Aborting due to memory budget exceeded after data load.\n");
    exit(1);
  }

  chrs = chrs_list;

  // Load BED annotations (enhancer, promoter, epigenetic) if configured
  loadAnnotations();
}

// ---------------------------------------------------------------------------
// setContactDataFromMemory — bypass file I/O, feed parsed data directly
// ---------------------------------------------------------------------------
inline void LooperSolver::setContactDataFromMemory(
    std::vector<string> chrs_list,
    BedRegion region_of_interest,
    const std::vector<MergedAnchor> &merged_anchors,
    const std::vector<MergedLoop> &merged_loops,
    const std::string &factor_name) {

  printf("set contact data from memory (%d anchors, %d loops)\n",
         (int)merged_anchors.size(), (int)merged_loops.size());
  ...
  if (!data_loaded) {
    arcs.loadAnchorsFromMemory(merged_anchors);
    arcs.loadArcsFromMemory(merged_loops, factor_name, segments_predefined);
...
inline void LooperSolver::loadAnnotations() {
  // Helper lambda: read a BED file and mark overlapping anchors with the given type.
  // BED format: chr\tstart\tend (tab-delimited, 0-based half-open)
  auto markFromBed = [&](const std::string &bed_path, AnnotationType atype) {
    if (bed_path.empty())
      return;
    FILE *f = fopen(bed_path.c_str(), "r");
    if (!f) {
      printf("[ANNOT] WARNING: cannot open annotation file %s\n", bed_path.c_str());
      return;
    }
    printf("[ANNOT] Loading annotations from %s (type=%d)\n", bed_path.c_str(), (int)atype);

    // Read all BED regions into a per-chromosome sorted vector
    struct BedEntry { int start; int end; };
    std::map<std::string, std::vector<BedEntry>> bed_regions;

    char chr_buf[256];
    int bstart, bend;
    while (fscanf(f, "%255s %d %d", chr_buf, &bstart, &bend) == 3) {
      // Skip rest of line (extra columns)
      int c;
      while ((c = fgetc(f)) != '\n' && c != EOF) {}
      bed_regions[std::string(chr_buf)].push_back({bstart, bend});
    }
    fclose(f);

    // Sort regions per chromosome for binary search
    for (auto &kv : bed_regions) {
      std::sort(kv.second.begin(), kv.second.end(),
                [](const BedEntry &a, const BedEntry &b) { return a.start < b.start; });
    }

    // For each chromosome's anchors, check overlap with BED regions
    int marked = 0;
    for (const auto &chr : arcs.chrs) {
      if (bed_regions.find(chr) == bed_regions.end())
        continue;
      const auto &regions = bed_regions[chr];
      if (arcs.anchors.find(chr) == arcs.anchors.end())
        continue;
      const auto &chr_anchors = arcs.anchors[chr];

      for (size_t ai = 0; ai < chr_anchors.size(); ++ai) {
        int amid = chr_anchors[ai].center;
        // Binary search for the first region that could overlap
        // A BED region [rs, re) overlaps anchor midpoint if rs <= amid < re
        int lo = 0, hi = (int)regions.size() - 1;
        bool found = false;
        while (lo <= hi) {
          int mid = (lo + hi) / 2;
          if (regions[mid].end <= amid) {
            lo = mid + 1;
          } else if (regions[mid].start > amid) {
            hi = mid - 1;
          } else {
            found = true;
            break;
        ...
        if (found) {
          // Store annotation in anchor_annotations map; will be applied to
          // clusters during createTreeChromosome().
          marked++;
          anchor_annotations[chr].resize(chr_anchors.size(), ANNOT_NONE);
          // Don't overwrite higher-priority annotations (e.g. CTCF)
          if (anchor_annotations[chr][ai] == ANNOT_NONE)
            anchor_annotations[chr][ai] = atype;
        }
      }
    }
    printf("[ANNOT] Marked %d anchors with annotation type %d\n", marked, (int)atype);
  };

  // First, mark CTCF anchors based on existing orientation data
  for (const auto &chr : arcs.chrs) {
    if (arcs.anchors.find(chr) == arcs.anchors.end())
      continue;
    const auto &chr_anchors = arcs.anchors[chr];
    anchor_annotations[chr].resize(chr_anchors.size(), ANNOT_NONE);
    for (size_t ai = 0; ai < chr_anchors.size(); ++ai) {
      if (chr_anchors[ai].orientation == 'L' || chr_anchors[ai].orientation == 'R') {
        anchor_annotations[chr][ai] = ANNOT_CTCF;
      }
    }
  }

  // Load BED annotation files
  markFromBed(Settings::enhancerAnnotationFile, ANNOT_ENHANCER);
  markFromBed(Settings::promoterAnnotationFile, ANNOT_PROMOTER);
  markFromBed(Settings::epigeneticAnnotationFile, ANNOT_EPIGENETIC);
}

inline void LooperSolver::getSnapshot(string s) {
  return;
  chromosome_set.add(getCurrentChromosomes(), s);
}

inline void LooperSolver::addSnapshot(Chromosome chr, string s) {
  // chromosome_set.add(chr, s);
  // chr_set.add(getCurrentChromosome(), s);
}

inline std::map<std::string, Chromosome> LooperSolver::getCurrentChromosomes() {
  HierarchicalChromosome hc = getModel();
  hc.createCurrentLevelStructure();
  return hc.chr;
}

inline HierarchicalChromosome LooperSolver::getModel() {
  HierarchicalChromosome hc;
...
inline int LooperSolver::findUnloopedChromatinLength(int cluster) {

  printf("find unlooped %d\n", cluster);
  int r = 0;
  int arcs = 0; // arcs cnt
  for (int i = clusters[cluster].base_start; i <= clusters[cluster].base_end;
       ++i) {
    printf("base %d %d\n", i, arcs);
    for (size_t j = 0; j < base_clusters[i].siblings.size(); ++j) {
      // printf("arc %d\n", base_clusters[i].siblings[j]);
      if (base_clusters[i].siblings[j] > i)
        arcs++;
      else
        arcs--;
    }
...
inline void LooperSolver::densify() {

  // go through all leaves. if span between two is too large, add additional
  // clusters every 'avg' bp
  ...
  // find average span
  float avg = 0.0f;
  for (int i = 1; clusters[i].level == base_level;
       ++i) { // go through all base level clusters
  ...
  for (int i = 1; clusters[i].level == base_level;
    ...
    if (d > lim) {
      printf("large span: %d %d (%d %d)\n", i, d, clusters[i - 1].genomic_pos,
             clusters[i].genomic_pos);

      // now add 'n' clusters between clusters 'i' and 'i-1'. set their parents
      // to parents of 'i' and 'i-1', whichever is closer

      int add = static_cast<int>(floor(d / avg)); // how many clusters to add in between
      int shift = d / add;      // distance between succesive clusters
      int middle =
          clusters[i - 1].end + d / 2; // find the middle point (used to decide
                                       // to which parent to add)
      ...
      for (int j = 0; j < add; ++j) {
        ...
        Cluster c(p, p);
        // c.pos = interpolate(clusters[active_region[i]].pos,
        // clusters[active_region[i+1]].pos, st);
        c.level = base_level;
        c.parent = parent;
        ...
        if (p > middle && before_middle) {
          ...
          clusters[parent].children.insert(clusters[parent].children.end(),
        ...
        children.push_back(static_cast<int>(clusters.size()) - 1);

        c.print();
      }

      printv(children, true, true, "insert to beg");
      clusters[parent].children.insert(clusters[parent].children.begin(),
                                       children.begin(), children.end());
    }
  }
}

inline std::vector<int> LooperSolver::findGaps(string chr) {
  std::vector<int> gaps;

  // for every anchor we want to count how many arcs are "above" it. if it's 0,
  // than we have a gap
  ...
  for (size_t i = start; i < clusters.size(); ++i) {
    for (size_t j = 0; j < clusters[i].arcs.size();
         ++j) { // check all arcs going in/out
      int other_end = otherEnd(chr, clusters[i].arcs[j], static_cast<int>(i));

      // printf("%d %d   (%d %d) %d\n", i, j, clusters[i].arcs[j], other_end,
      // arcs_cnt);
      if ((int)i == other_end)
        continue; // shouldn't happen

      if (other_end > (int)i)
        arcs_cnt++; // check whether we start or end an arc
      else
        arcs_cnt--;
    }

    if (arcs_cnt == 0) {
      gaps.push_back(static_cast<int>(i));
      // printf("gap %d %d\n", i, (clusters[i+1].start+clusters[i].end)/2);
      // if (i+1 < base_clusters.size()) vector_insert_unique(gaps_pos,
      // (base_clusters[i].end + base_clusters[i+1].start) / 2);
...
// find the optimal split of gaps, so that the bins have approximately size of
// 'exp_size' 'gaps' should start with 0 (gaps_pos = leftmost position) and end
// with index of last gap (gap pos = rightmost pos)
inline std::vector<int> LooperSolver::findSplit(std::vector<int> gaps, int exp_size,
                                         string chr) {
  ...
  for (size_t i = 1; i < gaps.size(); ++i) {
    ...
    if (i + 1 < gaps.size())
      S.push_back(clusters[gaps[i] + 1].start - clusters[gaps[i]].end);
  }
...
inline void LooperSolver::createTreeGenome() {

  // create a true root cluster
  // root has no genomic position, as it includes (potentialy) different
  // chromosomes for the same reason it doesn't have a base start/end
  // Cluster root;
  // root.level = 0;
  ...
  for (string chr : chrs) {
    ...
    // root.children.push_back(chr_root_index);	// add root of chr tree as a
    // child
  }
  ...
  // clusters[root.children[i]].parent = clusters.size();
  // clusters.push_back(root);
}

inline int LooperSolver::createTreeChromosome(string chr) {
  int cluster_start =
      static_cast<int>(clusters.size()); // index of first cluster for a given chromosome
  ...
  for (int i = 0; i < arcs.anchors_cnt[chr]; ++i) {
    ...
    c.level = curr_level;
    // Apply annotation type from BED annotations loaded in loadAnnotations()
    if (anchor_annotations.find(chr) != anchor_annotations.end() &&
        i < (int)anchor_annotations[chr].size()) {
      c.annotation_type = anchor_annotations[chr][i];
    }
    clusters.push_back(c);
  }
  ...
  //    gaps: [total: 49]  18950 18956 18962 18968 18982 18991 19001 19005 19014
  //    19031 19038 19046 19065 19073 19085 19087 19091 19102 19109 19126 19133
  //    19134 19135 19139 19156 19165 19172 19181 19183 19191 19198 19205 19213
  //    19230 19267 19274 19276 19295 19299 19302 19304 19309 19317 19334 19346
  //    19354 19360 19363 19366 splits: [total: 31]  18950 18962 18982 18991
  //    19001 19014 19038 19046 19065 19073 19087 19091 19102 19109 19133 19139
  //    19156 19181 19205 19213 19230 19267 19276 19299 19304 19309 19317 19334
  //    19346 19354 19366
  // gaps refers to interaction blocks, splits - to segments
  // to create clusters for interaction blocks (ib) we simply include all
  // clusters between consecutive gaps to create clusters for segments we need
  // to refer to ib indices.

  // int first_gap_index = clusters.size(); // index of first ib cluster (used
  // when creating segment clusters)

  int next_split_ind = 1;
  int start_gap_index = static_cast<int>(clusters.size());
  ...
  for (size_t i = 1; i < gaps.size(); ++i) {
    ...
    int prev_gap = i == 1
                       ? gaps[i - 1]
                       : gaps[i - 1] + 1; // boundary gaps (both are inclusive)
    int curr_gap = gaps[i];
    ...
    // curr_gap, start_pos, end_pos);

    Cluster c(
        start_pos,
        end_pos); // interaction block cluster (anywhere from 100kb to ~Mb)
    ...
    for (int k = prev_gap; k <= curr_gap; ++k) {
      ...
      clusters[k].parent = static_cast<int>(clusters.size());
    }
    ...
    if (gaps[i] == splits[next_split_ind]) {
      ...
      int end_gap_index = static_cast<int>(clusters.size()) - 1;

      Cluster cs(clusters[start_gap_index].start,
                 clusters[end_gap_index].end); // segment cluster
      ...
      int segment_cluster_ind =
          static_cast<int>(clusters.size()); // cluster-index that will be given to segment
                           // cluster that is currently being created
      ...
      start_gap_index = static_cast<int>(clusters.size());
      next_split_ind++;
    }
  }
  ...
  curr_level -= 2;
  int root_ind = static_cast<int>(clusters.size()); // save root index

  rootc.level = curr_level;
  rootc.start = clusters[rootc.children[0]].start;
  rootc.end = clusters[rootc.children[rootc.children.size() - 1]]
                  .end; // TODO: was start, should be?
  ...
  for (size_t i = 0; i < rootc.children.size(); ++i)
    clusters[rootc.children[i]].parent = static_cast<int>(clusters.size());
  clusters.push_back(rootc);
  return root_ind;
}

// set top level of a structure - a root for every chromosome
inline void LooperSolver::setTopLevel() {
  current_level.clear();
  for (string chr : chrs) {
...
inline void LooperSolver::setLevel(int level) {
  setTopLevel();
  while (level--)
    levelDown();
}

inline Heatmap LooperSolver::downsampleSingletonHeatmap(Heatmap &heat) {
  ...
  std::map<std::string, std::vector<int>>
      children_ind; // index of first child for every chromosome
  for (string chr : chrs) {
    children_ind[chr].push_back(0);
    for (size_t i = 0; i < current_level[chr].size(); ++i)
      children_ind[chr].push_back(
          children_ind[chr][i] +
          static_cast<int>(clusters[current_level[chr][i]].children.size()));
  }
  // for (int i = 0; i < n; ++i) children_ind.push_back(children_ind[i] +
  // clusters[current_level[i]].children.size());
    ...
    for (string chrB : chrs) {
      ...
      for (size_t i = 0; i < cntA; ++i) {
        for (size_t j = i; j < cntB; ++j) {
          ...
          // (cell (i,j) of current region correspond to a square subset of
          // smaller heatmap: [a,b]x[p,q])
          ...
          for (size_t k = 0; k < cnt; ++k) {
            for (size_t l = 0; l < cnt2; ++l) {
              ...
              // chrB.c_str(), i, j, k, l, indA, indB);
              avg +=
                  heat.v[children_ind[chrA][i] + k][children_ind[chrB][j] + l];
            }
          }
          avg /= cnt * cnt2;
      ...
      indB += static_cast<int>(cntB);
    }
    indA += static_cast<int>(cntA);
  }
  h.clearDiagonal(1);
  return h;
}

// if selected region is small, it may happen that it is contained within one
// segment level bead. Then we can not create a heatmap
inline Heatmap LooperSolver::createSingletonHeatmap(int diag) {

  // find boundary positions for bins
  std::map<std::string, std::vector<int>> breaks;
  std::map<std::string, int> start_ind; // starting index for chromosomes (index
                                        // of first column in a heatmap)
  int pos;
  ...
  for (string chr : chrs) {
    ...
    for (size_t i = 0; i + 1 < current_level[chr].size(); i++) {
      pos = (clusters[current_level[chr][i]].end +
             clusters[current_level[chr][i + 1]].start) /
            2;
      breaks[chr].push_back(pos);
    }
    // breaks[chr].push_back()
    breaks[chr].push_back(1000000000);

    start_ind[chr] = curr_ind;
    curr_ind += static_cast<int>(breaks[chr].size()) - 1;
  }
  ...
  // create a set of chromosomes to speed up checking if chromosome is to be
  // processed
  std::set<std::string> chrs_set;
  for (std::string chr : chrs)
    chrs_set.insert(chr);

  const int max_position = 2000000000;

  // we need to keep min and max genomic position for every chromosome (to
  // calculate lengths of first and last bin)
  ...
  singleton_files.insert(singleton_files.end(), arcs_singletons.begin(),
                         arcs_singletons.end());
  if (chrs.size() > 1)
    singleton_files.insert(singleton_files.end(), arcs_singletons_inter.begin(),
                           arcs_singletons_inter.end());

  // if split by chr and single chromosome, then update names (update is a bit
  // more problematic for multiple chromosomes)
  if (Settings::dataSplitSingletonFilesByChr && chrs.size() == 1) {
    ...
    for (uint i = 0; i < singleton_files.size(); ++i) {
      ...
      singleton_files[i] =
          ftext("%schr/%s.%s", path.c_str(), fname.c_str(), chrs[0].c_str());
    }
  }
  ...
  for (uint fi = 0; fi < singleton_files.size(); ++fi) {
    ...
    int cnt_arcs_bad_chr = 0; // number of arcs on other chromosomes (not
                              // specified with -c or chrM)
    ...
    while (fgets(singleton_line, sizeof(singleton_line), f) != NULL) {
      ...
      if (g_cancel_requested.load(std::memory_order_relaxed)) {
        printf("\n[CANCEL] createSingletonHeatmap interrupted\n");
        break;
      }

      // REQ-3.2: parse from line buffer instead of direct fscanf
      int args_read = sscanf(singleton_line, "%s %d %d %s %d %d %d", chr1, &sta, &stb, chr2,
                             &enda, &endb, &sc);
      if (args_read < 7) {
        if (args_read > 0)
          cnt_lines_err++;
      ...
      if (chrs_set.find(chr1) == chrs_set.end() ||
          chrs_set.find(chr2) == chrs_set.end()) {
        cnt_arcs_bad_chr++;
        continue;
      }
      ...
      if (is_region_selected &&
          (!selected_region.contains(sta) || !selected_region.contains(endb)))
        continue;
      ...
      // 1. find correct index within a chromosome by iterating over breaks for
      // this chromosome
    ...
    printf("\n%d interactions added, ignored %d (bad chr) and %d (diagonal)",
           cnt_arcs_ok, cnt_arcs_bad_chr, cnt_arcs_diag);
    if (cnt_lines_err > 0)
      printf(", failed to read %d lines", cnt_lines_err);
    printf("\n");
  }
    ...
    for (InteractionArc arc : arcs.long_arcs[chr]) {
      ...
      for (size_t i = 0; i + 1 < breaks[chr].size(); ++i) {
        if (breaks[chr][i] <= sta && sta <= breaks[chr][i + 1]) {
          st = start_ind[chr] + static_cast<int>(i);
          break;
        }
      }
      for (size_t i = 0; i + 1 < breaks[chr].size(); ++i) {
        if (breaks[chr][i] <= enda && enda <= breaks[chr][i + 1]) {
          end = start_ind[chr] + static_cast<int>(i);
          break;
        }
      }
      ...
      float val = static_cast<float>(Settings::longPETClustersEffectScale *
                  pow(arc.score, Settings::longPETClustersEffectPower));
  ...
  for (string chr : chrs) {
    for (size_t i = 0; i + 1 < breaks[chr].size(); ++i) {
      int d = getGenomicLengthExcludingCentromere(chr, breaks[chr][i],
                                                  breaks[chr][i + 1]);
      len.push_back(static_cast<float>(d) / 1e6f);
    }
  }
...
inline void LooperSolver::createSingletonSubanchorHeatmap(
    string chr, vector<int> &anchors_gap_len) {
  int anchor_margin = 0;
  ...
  int region_end =
      clusters[active_region[active_region.size() - 1]].end + anchor_margin;

  printf("heatmap boundary: %d %d\n", region_start, region_end);

  float dt = 1.0f / (Settings::loopDensity);
  breaks.push_back(region_start);
  anchors_gap_len.push_back(clusters[active_region[0]].end -
                            clusters[active_region[0]].start);
  ...
  for (size_t i = 1; i < active_region.size(); i++) {
    ...
    int anchor_len =
        clusters[active_region[i]].end - clusters[active_region[i]].start;
    ...
    for (int j = 0, t = static_cast<int>(dt); j < Settings::loopDensity - 1; j++, t += static_cast<int>(dt)) {
  ...
  heatmap_anchor.init(static_cast<int>(active_region.size()));
  heatmap_subanchor.init(static_cast<int>(breaks.size()) - 1);
  ...
  for (size_t fi = 0; fi < arcs_singletons.size(); ++fi) {
    ...
    if (Settings::dataSplitSingletonFilesByChr) {
      ...
      file_chr = arcs_singletons[fi];
    printf("read [%s]\n", file_chr.c_str());

    f = open(file_chr.c_str(), "r");
    if (!f) {
      if (Settings::dataSplitSingletonFilesByChr)
        error("No chromosome-splitted file found!");
      error("could not read the file!");
    }
    ...
    while (!feof(f)) {
      ...
      // 1: chr9	77361535	77361615	chr9	78624184
      // 78624253	1 2: chr18	51650936	51651093	chr3
      // 110168590	110168697	2	0	0 3: chr6	34306237
      // 34306388	chrX	109134352	109134456	1	+
      // + we read first 7 column, and then fgets() to reach the end of line

      if (fscanf(f, "%s %d %d %s %d %d %d", chr1, &sta, &stb, chr2, &enda,
                 &endb, &sc) < 7) {
        cnt_lines_err++; // something went wrong (possibly empty last line in a
                         // file)
        continue;
      }
      ...
      // check specified region
      if (st_pos < region_start || end_pos > region_end)
        continue;
      ...
      for (size_t i = 1; i < breaks.size() && (st == -1 || end == -1); ++i) {
        if (st == -1 && st_pos <= breaks[i])
          st = static_cast<int>(i) - 1;
        if (end == -1 && end_pos <= breaks[i])
    ...
    fclose(f);
    printf("%d interactions added, %d ignored (diagonal); failed to read %d "
           "lines\n",
           cnt_arcs_ok, cnt_arcs_diag, cnt_lines_err);
  }

  int n = static_cast<int>(heatmap_subanchor.size);

  vector<bool> is_anchor(n, false);
  for (int i = 0; i < n; ++i)
    if (i % (Settings::loopDensity + 1) == 0)
      is_anchor[i] = true;

  // create anchor heatmap
  float val;
  int n_anchor = static_cast<int>(heatmap_anchor.size);
  for (int i = 0; i < n_anchor; ++i) {
    for (int j = i + 1; j < n_anchor; ++j) {
      val = heatmap_subanchor.v[i * (Settings::loopDensity + 1)]
                               [j * (Settings::loopDensity + 1)];
      val /= (anchors_gap_len[2 * i] * anchors_gap_len[2 * j]) /
             1000000.0f; // normalize by anchor length
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = i + 1; j < n; ++j) {
      if (is_anchor[i] && is_anchor[j])
        end_anchor += heatmap_subanchor.v[i][j];
      else if (!is_anchor[i] && !is_anchor[j])
        end_loop += heatmap_subanchor.v[i][j];
      else
        end_loop_anchor += heatmap_subanchor.v[i][j];
    }
  }

  float tot = end_anchor + end_loop_anchor + end_loop;
  printf("types [anchor/loop]: %.0f %.0f %.0f\n", end_anchor, end_loop_anchor,
         end_loop);
  ...
  for (int i = 0; i < n; ++i) {
    ...
    i1 *= 2;
    d1 = i % (Settings::loopDensity + 1) == 0
             ? anchors_gap_len[i1]
             : ((float)anchors_gap_len[i1 + 1] / Settings::loopDensity);
    d1 /= 1000.0f;
    printf("%f, ", d1);
    for (int j = i + 1; j < n; ++j) {
      i2 = j / (Settings::loopDensity + 1);
      i2 *= 2;
      d2 = j % (Settings::loopDensity + 1) == 0
               ? anchors_gap_len[i2]
               : ((float)anchors_gap_len[i2 + 1] / Settings::loopDensity);
      d2 /= 1000.0f;
...
inline Heatmap LooperSolver::normalizeHeatmap(const Heatmap &heat) {
  int i, j;
  int n = static_cast<int>(heat.size);
  Heatmap h;
  h.init(n);
  h.diagonal_size = heat.diagonal_size;

  // assumption: total number of reads should be equal for every region
  // find expected number of reads and multiply every row by R[i] so that the
  // new sum is equal to the expected as expected number of reads use the
  // average
  ...
  for (i = 0; i < n; ++i) { // for every row...
    float sum = 0.0f;
    for (j = 0; j < n; ++j)
      sum += heat.v[i][j]; // ... find the sum of counts
    expected_sum += sum;
  }
...
inline void LooperSolver::createDistanceHeatmap(int heatmap_ind) {
  int n = static_cast<int>(heatmap[heatmap_ind].size);
  heatmap_dist.init(n);
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = i; j < n; ++j) {
      ...
      if (val < 1e-6)
        heatmap_dist.v[i][j] = 0.0f;
      else {
        heatmap_dist.v[i][j] =
            (abs(i - j) < heatmap_dist.diagonal_size)
                ? -1.0f
                : (heatmap_ind == 0 ? freqToDistanceHeatmapInter(val)
                                    : freqToDistanceHeatmap(val));
      }
...
inline void LooperSolver::levelDown() {
  std::vector<int> tmp;
  for (string chr : chrs) {
    for (size_t i = 0; i < current_level[chr].size(); ++i) {
      ...
      if (clusters[current_level[chr][i]].children.size() == 0) {
        ...
        for (size_t j = 0; j < clusters[current_level[chr][i]].children.size();
             ++j) {
...
inline void LooperSolver::normalizeHeatmapInter(Heatmap &heat, float scale) {
  if (chrs.size() <= 1)
    return; // there is only one chr, scaling inter has no sense
  ...
  for (size_t j = 0; j < chrs.size(); j++) {
    chr = chrs[j];
    start_ind[chr] = curr_ind;
    curr_ind += static_cast<int>(current_level[chr].size());
  }
...
inline void LooperSolver::normalizeHeatmapDiagonalTotal(Heatmap &heat, float val) {
  ...
  double avg = 0.0;
  for (size_t i = 0; i + diag < heat.size; ++i)
    avg += heat.v[i][i + diag];
  avg /= heat.size - diag;
...
inline float LooperSolver::getHeatmapAvgBetweenNeighboringRegions(Heatmap &heat,
                                                           int level) {
  int i, j, k;
  int n;
  ...
  for (string chr : chrs) {
    ...
    vector<int> v;
    v.push_back(0);
    for (size_t l = 0; l < current_level[chr].size(); ++l)
      v.push_back(v[l] + static_cast<int>(clusters[current_level[chr][l]].children.size()));
...
inline double LooperSolver::calcScoreDistancesActiveRegion() {
  double sc = 0.0, diff;
  int i, j;
  ...
  for (i = 0; i < n; ++i) {
    for (j = i + 1; j < n; ++j) {
      ...
      if (heatmap_exp_dist_anchor.v[i][j] < 1e-6)
        continue; // ignore near-zero values (instability)

      diff = (v.length() - heatmap_exp_dist_anchor.v[i][j]) /
             heatmap_exp_dist_anchor.v[i][j];
      // printf("%d %d  -> (%lf %lf) %lf\n", i, j, v.length(),
      // heatmap_exp_dist_anchor.v[i][j], diff);
      sc += diff * diff *
...
// calculate score based on distances between the selected bead and clusters
// connected with it by arcs
inline double LooperSolver::calcScoreDistancesActiveRegion(int cluster_moved) {
  double sc = 0.0, diff;
  int i, st;
  ...
  for (i = 0; i < n; ++i) {
    ...
    if (heatmap_exp_dist_anchor.v[i][cluster_moved] < 1e-6)
      continue; // anchors not connected by an arc are denoted by -1

    diff = (v.length() - heatmap_exp_dist_anchor.v[i][cluster_moved]) /
           heatmap_exp_dist_anchor.v[i][cluster_moved];
    // printf("%d %d (%d %d)  -> (%lf %lf) %lf\n", i, cluster_moved, st,
    // active_region[i], v.length(),
    // heatmap_exp_dist_anchor.v[i][cluster_moved], diff);
    sc += diff * diff *
          (diff >= 0.0f ? Settings::springConstantStretchArcs
                        : Settings::springConstantSqueezeArcs);
  }
...
// ============================================================================
// Loop Energy: E_loop = sum_constraints k_loop * (r_ij - r0)^2
// ============================================================================

inline void LooperSolver::loadLoopConstraints(const std::string &filepath) {
  loop_constraints.clear();
  if (filepath.empty()) return;

  FILE *f = fopen(filepath.c_str(), "r");
  if (!f) {
    printf("Warning: cannot open loop constraints file: %s\n", filepath.c_str());
    return;
  }
  ...
  while (fgets(line, sizeof(line), f)) {
    ...
    LoopConstraint lc;
    float k = Settings::loopStiffness;
    float r0 = Settings::loopEquilibriumDistance;

    int nread = sscanf(line, "%d %d %f %f", &lc.cluster_i, &lc.cluster_j, &k, &r0);
    if (nread < 2) continue;

    lc.stiffness = k;
    lc.eq_distance = r0;
    loop_constraints.push_back(lc);
    count++;
  }
  fclose(f);
  printf("Loaded %d loop constraints from %s\n", count, filepath.c_str());
}

inline void LooperSolver::rebuildActiveLoopMap() {
  active_loop_map.clear();

  // Build reverse map: global cluster index -> active region index
  std::map<int, int> cluster_to_active;
  for (int i = 0; i < (int)active_region.size(); ++i) {
    cluster_to_active[active_region[i]] = i;
  }

  for (int c = 0; c < (int)loop_constraints.size(); ++c) {
    auto it_i = cluster_to_active.find(loop_constraints[c].cluster_i);
    auto it_j = cluster_to_active.find(loop_constraints[c].cluster_j);
    if (it_i != cluster_to_active.end() && it_j != cluster_to_active.end()) {
      int ai = it_i->second;
      int aj = it_j->second;
      auto key = (ai < aj) ? std::make_pair(ai, aj) : std::make_pair(aj, ai);
      active_loop_map[key] = c;
    }
  }
}

inline double LooperSolver::calcScoreLoopEnergy() const {
  double sc = 0.0;
  for (const auto &entry : active_loop_map) {
    int ai = entry.first.first;
    int aj = entry.first.second;
    const LoopConstraint &lc = loop_constraints[entry.second];

    vector3 v = clusters[active_region[ai]].pos -
                clusters[active_region[aj]].pos;
    double d = v.length();
    double delta = d - lc.eq_distance;
    sc += lc.stiffness * delta * delta;
  }
  return sc;
}

inline double LooperSolver::calcScoreLoopEnergy(int cluster_moved) const {
  double sc = 0.0;
  for (const auto &entry : active_loop_map) {
    int ai = entry.first.first;
    int aj = entry.first.second;
    if (ai != cluster_moved && aj != cluster_moved)
      continue;
...
inline double LooperSolver::calcScoreStructureActiveRegionLengths() {
  double sc = 0.0;
  double diff, dtn;
  vector3 v;
  for (size_t i = 0; i + 1 < active_region.size(); i++) {
    ...
    sc += diff * diff *
          (diff >= 0.0f ? Settings::springConstantStretch
                        : Settings::springConstantSqueeze);
  }
  return sc;
}

// calculate score based on distances to previous and next cluster
inline double LooperSolver::calcScoreStructureActiveRegionLengths(int cluster_moved) {
  double sc = 0.0;
  double diff, dtn;
  vector3 v;
  for (int i = cluster_moved - 1; i <= cluster_moved; i++) {
    if (i < 0 || i + 1 >= (int)active_region.size())
      continue;
...
inline double LooperSolver::calcScoreStructureSmooth(bool lengths, bool angles) {
  double diff, ang, dtn;
  vector3 v, v2;
  ...
  for (size_t i = 0; i + 1 < active_region.size(); i++) {
    ...
    if (angles && i > 0) {
      ...
      // printf("sm %d %lf %f\n", i, ang * ang *
      // Settings::springAngularConstant, ang);
    }

    // printf("diff = %lf, ang=%lf   %f %f %f  (%f %f %f)\n", diff, ang, v.x,
    // v.y, v.z, 		clusters[active_region[i]].pos.x,
    //clusters[active_region[i]].pos.y, clusters[active_region[i]].pos.z);
    v2 = v; // now, in the next loop, we will have two vectors and so we can
            // calculate angles
  }
...
inline double LooperSolver::calcScoreStructureSmooth(int cluster_moved, bool lengths,
                                              bool angles) {
  double diff, ang, dtn;
  vector3 v, v2;
...
inline double LooperSolver::calcScoreOrientation(const vector<vector3> &orientation) {
  double ang, err = 0.0;
  for (auto el : active_anchors_neighbors) {
    int n = static_cast<int>(el.second.size());
    for (int i = 0; i < n; ++i) {
      ang = angle_norm(orientation[el.first],
                       static_cast<float>(Settings::motifsSymmetric ? 1.0 : -1.0) *
                           orientation[el.second[i]]);
...
inline double LooperSolver::calcScoreOrientation(const vector<vector3> &orientation,
                                          int anchor_index) {
  double err = 0.0, ang;
  if (active_anchors_neighbors.find(anchor_index) ==
      active_anchors_neighbors.end()) {
  ...
  // consider whether the symmetric or assymetric paradigm is chosen (by
  // possibly reversing motif orientation)
  for (int val : active_anchors_neighbors[anchor_index]) {
    ang =
        angle_norm(orientation[anchor_index],
                   static_cast<float>(Settings::motifsSymmetric ? 1.0 : -1.0) * orientation[val]);
    err += ang * ang;
  }

  //	if (anchor_index > 0) {
  //		ang = angle_norm(orientation[anchor_index],
  //orientation[anchor_index-1]); 		err += ang * ang;
  //	}
...
// - check if clustered moved actually modified orientation (to do so, it must
// be an anchor or a direct neighbor)
...
//
//	// during subanchor level simulation anchors have 'is_fixed' flag set.
//we can use it to identify anchors
//
...
// moved - index of a point last moved. if none is specified returns sum of
// score for all points
inline double LooperSolver::calcScoreHeatmapActiveRegion(int moved) {
  double err = 0.0, cerr;
  float d;

  size_t n = active_region.size();
  if (heatmap_dist.size != n)
    error(ftext("heatmap sizes mismatch, dist size=%d, active region=%d",
                heatmap_dist.size, n));

  if (moved == -1) {
    for (size_t i = 0; i < n; ++i)
      err += calcScoreHeatmapActiveRegion(static_cast<int>(i));
  } else {
    int st = 0;
    int end = static_cast<int>(n) - 1;
    if (heatmap_chromosome_boundaries.size() > 0)
      getChromosomeHeatmapBoundary(moved, st, end);

    for (int i = st; i <= end; ++i) {
      if (abs(i - moved) >= heatmap_dist.diagonal_size) {

        if (heatmap_dist.v[i][moved] < 1e-6)
          continue; // ignore 0 values

        d = (clusters[active_region[i]].pos -
             clusters[active_region[moved]].pos)
                .length();
...
inline void LooperSolver::updateDensity() {
  // clear density_curr
  // for all points:
  ...
  // vector3 origin = density.center - vector3(density. *
  // Settings::densityScale;
  int px, py, pz;
  for (size_t i = 0; i < n; ++i) {

    // find 3d density-coordinates for the current bead
    // pos = (clusters[active_region[i]].pos - density.center) *
    // Settings::densityScale + shift;
    pos = (clusters[active_region[i]].pos - density.center) *
          Settings::densityScale;
    pos.x /= 3.0f;
    pos += shift;
    ...
    if (px < 0 || py < 0 || pz < 0 || px >= density.size_x ||
        py >= density.size_y || pz >= density.size_z) {
      // printf("\npoint outside: %d %d %d\n", px, py, pz);
...
inline void LooperSolver::repairDensityBoundary() {
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    while (1) {
      ...
      if (px < 0 || py < 0 || pz < 0 || px >= density.size_x ||
        ...
        //				print_vector(clusters[active_region[i]].pos,
        //"pos"); 				print_vector(shift, "shift"); 				printf("%d %d %d\n", px, py,
        //pz);
...
inline double LooperSolver::calcScoreDensity() {
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if (px < 0 || py < 0 || pz < 0 || px >= density.size_x ||
      ...
      if (px < 0)
        d += -px;
      else if (px >= density.size_x)
        d += px - density.size_x + 1;
      if (py < 0)
        d += -py;
      else if (py >= density.size_y)
        d += py - density.size_y + 1;
      if (pz < 0)
        d += -pz;
      else if (pz >= density.size_z)
        d += pz - density.size_z + 1;
      ...
      // error += (clusters[active_region[i]].pos - density.center).length() *
      // 1000.0f; error += 100000; error += 100.0f * abs(min(py-15, pz-15));
  ...
  for (int i = 0; i < density.size_x; ++i) {
    for (int j = 0; j < density.size_y; ++j) {
      for (int k = 0; k < density.size_z; ++k) {
        ...
        // e = (density.t[i][j][k] > epsilon) ? (density.t[i][j][k] -
        // density_curr.t[i][j][k]) / density.t[i][j][k] : 1000.0f *
        // density_curr.t[i][j][k];

        if (density.t[i][j][k] > epsilon) {
          e = (density.t[i][j][k] - density_curr.t[i][j][k]) /
              density.t[i][j][k];
          e = 0.0;
        } else {
        ...
        // e = (density.t[i][j][k] > epsilon) ? (density.t[i][j][k] -
        // density_curr.t[i][j][k]) / density.t[i][j][k] : 1000.0f; if
        // (density.t[i][j][k] < epsilon) printf("near zero: %lf  %lf\n",
...
inline double LooperSolver::calcScoreSubanchorHeatmap(int moved) {
  double err = 0.0, cerr;
  size_t n = active_region.size();
  ...
  if (moved == -1) {
    for (size_t i = 0; i < n; ++i) {
      err += calcScoreSubanchorHeatmap(static_cast<int>(i));
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      if (heatmap_dist_subanchor.v[i][moved] < 1e-6)
        continue; // ignore 0 values

      d = (clusters[active_region[i]].pos - clusters[active_region[moved]].pos)
              .length();
      cerr = (d - heatmap_dist_subanchor.v[i][moved]) /
             heatmap_dist_subanchor.v[i][moved];
      err += cerr * cerr;
      // printf("%d d=%f, exp_d=%f, %lf %lf\n", i, d,
      // heatmap_dist_subanchor.v[i][moved], cerr, err);
    }
...
inline void LooperSolver::densifyActiveRegion(int cluster, bool fix) {
  ...
  int level =
      clusters[active_region[0]].level + 1; // level for newly added points

  for (size_t i = 0; i + 1 < active_region.size(); ++i) {

    if (fix)
      clusters[active_region[i]].is_fixed = true;

    // get rightmost pos of left region and leftmost of right one
    int range =
        clusters[active_region[i + 1]].start - clusters[active_region[i]].end;
    int d = range / (add + 1);
    int p = clusters[active_region[i]].end;

    // update left anchor dist (because we insert some points that will be
    // closer)
    ...
    for (int j = 0; j < add; ++j) {
      ...
      tmp_active.push_back(static_cast<int>(clusters.size()) - 1);
  ...
  if (fix)
    clusters[active_region[active_region.size() - 1]].is_fixed = true;

  active_region = tmp_active;

  // we added new points thus modifying the structure tree, so we also need to
  // update the parent info
  clusters[cluster].children = active_region;
}

inline double LooperSolver::genomicLengthToDistance(int length) {
  return Settings::genomicLengthToDistBase +
         Settings::genomicLengthToDistScale *
             pow(length / 1000.0, Settings::genomicLengthToDistPower);
}

inline int LooperSolver::otherEnd(const InteractionArc &arc, int current_cluster) {
  if (arc.start == current_cluster)
    return arc.end;
  if (arc.end == current_cluster)
    return arc.start;
...
// returns index of cluster connected to 'current_cluster' by arc with index
// 'arc' Note that 'current_chr' must be properly set
inline int LooperSolver::otherEnd(int arc, int current_cluster) {
  if (arcs.arcs[current_chr][arc].start == current_cluster)
    return arcs.arcs[current_chr][arc].end;
  if (arcs.arcs[current_chr][arc].end == current_cluster)
    return arcs.arcs[current_chr][arc].start;
  return -1;
}

inline int LooperSolver::otherEnd(string chr, int arc, int current_cluster) {
  if (arcs.arcs[chr][arc].start == current_cluster)
    return arcs.arcs[chr][arc].end;
  if (arcs.arcs[chr][arc].end == current_cluster)
    return arcs.arcs[chr][arc].start;
  return -1;
}

inline float LooperSolver::freqToDistanceHeatmap(float freq) {
  return Settings::freqToDistHeatmapScale *
         pow(freq, Settings::freqToDistHeatmapPower);
}

inline float LooperSolver::freqToDistanceHeatmapInter(float freq) {
  return Settings::freqToDistHeatmapScaleInter *
         pow(freq, Settings::freqToDistHeatmapPowerInter);
}

inline float LooperSolver::distToFreqHeatmap(float freq) {
  return pow(freq / Settings::freqToDistHeatmapScale,
             1.0f / Settings::freqToDistHeatmapPower);
}
  ...
  return static_cast<float>(Settings::countToDistBaseLevel +
         Settings::countToDistScale /
             exp(Settings::countToDistA *
                 (double)(freq + Settings::countToDistShift)));
}
...
// - reconstruct anchor-level model (ie. using a single point for every anchor)
// (to get their proper 3D position)
// - densify model (put additional points between every two consecutive anchors)
// - refine subanchor-level model, keeping anchors fixed and moving only these
// additional points (to obtain nice loops)
inline void LooperSolver::reconstructClustersArcsDistances() {

  setLevel(LVL_SEGMENT); // set segment level

  // RMSD convergence tracking (F5.3): save initial checkpoint for arcs phase
  logRMSDCheckpoint("arcs_initial");
  ...
  for (string chr : chrs) {
    ...
    if (debug_chromosomes_limit > 0 && ii > debug_chromosomes_limit)
      break; // check chr number limit

    // REQ-5.1: cancellation check between chromosomes
    if (g_cancel_requested.load(std::memory_order_relaxed)) {
      printf("\n[CANCEL] reconstructClustersArcsDistances interrupted\n");
      return;
    }
    // REQ-4.1: memory budget check between chromosomes
    if (checkMemoryBudget(Settings::maxMemoryMB, "reconstructArcs chr loop")) {
      printf("[ERROR] Aborting arc reconstruction due to memory budget.\n");
      return;
    }
    logMemoryUsage(ftext("reconstructArcs chr %s", chr.c_str()).c_str());

    setLevel(LVL_SEGMENT); // set segment level

    if (Settings::outputLevel >= 6)
      printv(current_level[chr], true, true, "curr seg");
    ...
    if (Settings::outputLevel >= 6)
      printv(current_level[chr], true, true, "curr ib");

    // now go through all ibs
    curr_size = static_cast<int>(current_level[chr].size());
    for (int i = 0; i < curr_size; ++i) {

      // REQ-5.1: cancellation check between interaction blocks
      if (g_cancel_requested.load(std::memory_order_relaxed)) {
        printf("\n[CANCEL] IB reconstruction interrupted at %s %d/%d\n",
      ...
      active_region.assign(clusters[current_level[chr][i]].children.begin(),
                           clusters[current_level[chr][i]].children.end());
      if (Settings::outputLevel >= 6)
        printv(active_region, true, true, "active");
      ...
      for (size_t j = 0; j < active_region.size(); ++j)
        clusters[active_region[j]].pos = clusters[current_level[chr][i]].pos;

      // calc expected distances between anchors
      output(5, "calc exp distances for anchors\n");
      calcAnchorExpectedDistancesHeatmap();
      // heatmap_exp_dist_anchor.toFile(ftext("%sanchor_%s_%d_exp_dist.heat",
      // output_dir.c_str(), chr.c_str(), i));
      ...
      if (Settings::useSubanchorHeatmap) {
        ...
        subanchor_avg_dist.init(static_cast<int>(active_region.size()));

        // save current structure
        vector<vector3> str;
        for (size_t j = 0; j < active_region.size(); ++j)
          str.push_back(clusters[active_region[j]].pos);

        for (int j = 0; j < Settings::subanchorEstimateDistancesReplicates;
             ++j) {
          ...
          for (size_t j = 0; j < active_region.size(); ++j)
            clusters[active_region[j]].pos = str[j];
        }
        subanchor_avg_dist.scale(
            1.0f / Settings::subanchorEstimateDistancesReplicates);
        // subanchor_avg_dist.toFile(ftext("%sdist/subanchor_dist_%s_%d_avg.heat",
        // output_dir.c_str(), chr.c_str(), i));

        // create expected dist heatmap
        output(5, "create expected dist heatmap\n");
        createExpectedDistSubanchorHeatmap(subanchor_avg_dist);
        // heatmap_dist_subanchor.toFile(ftext("%sdist/subanchor_dist_%s_%d_exp.heat",
        // output_dir.c_str(), chr.c_str(), i));
      }
      ...
      //			if (Settings::useSubanchorHeatmap) {
      //				Heatmap hd =
      //calcTrueDistancesHeatmapForRegion(active_region);
      //				hd.toFile(ftext("%ssubanchor_dist_%s_%d_true.heat",
      //output_dir.c_str(), chr.c_str(), i));
      //			}
  ...
  if (Settings::outputLevel >= 2) {
    ...
    // Generate SVG chart for energy decomposition
    std::string energy_svg = energy_path.substr(0, energy_path.size() - 4) + ".svg";
    SvgChartGenerator::energyDecompositionChart(energy_path, energy_svg);
  }

  // RMSD convergence tracking (F5.3): compute RMSD vs initial after arcs phase
  {
    double rmsd = computeRMSDvsCheckpoint("arcs_initial");
    if (rmsd >= 0.0) {
      rmsd_convergence_log.push_back({"arcs_final", rmsd});
      output(1, "[RMSD] Arcs phase: RMSD vs initial = %.4f\n", rmsd);
    }
    writeRMSDConvergenceCSV(label);
  }
}
...
// if there is only one segment we can use random walk, as we have no other
// reference
inline void LooperSolver::positionInteractionBlocks(std::vector<int> &segments) {

  if (segments.size() > 1)
    interpolateChildrenPositionSpline(segments, true);
  else {
    // TODO: create heatmap for ib?
    printf("only one segment, use random walk to position ib (step size=%f)\n",
           Settings::ibRandomWalkJumps);
    int n = static_cast<int>(clusters[segments[0]].children.size());
    rw_pos.set(0.0f, 0.0f, 0.0f);
...
// cluster_ind - parent index of current_level array (needed to get
// previous/next clusters for interpolation) smooth - if false then use anchor
// level parameters (ie. use proper energy function), otherwise sub-anchor level
// active_region should contain indices of all
// Note: function assumes that 'current_chr' stores the id of chromosome that
// 'cluster' belongs to
inline void LooperSolver::reconstructClusterArcsDistances(int cluster, int cluster_ind,
                                                   bool smooth,
                                                   bool use_subanchor_heatmap) {
  // printf("reconstruct cluster %d (current_region ind: %d, smooth=%d)\n",
  // cluster, cluster_ind, smooth); clusters[cluster].print();
  ...
  if (active_size == 0) {
    error(ftext("active size == 0!\ncluster=%d, cluster ind=%d, smooth=%d",
  ...
  if (smooth) {
    if (use_subanchor_heatmap && heatmap_dist_subanchor.size != active_size)
      error(ftext("heatmap_dist_subanchor size mismatch (%d, should be %d)",
                  heatmap_dist_subanchor.size, active_size));
  } else {
    if (heatmap_exp_dist_anchor.size != active_size)
      error(ftext("heatmap_exp_dist_anchor size mismatch (%d, should be %d)",
                  heatmap_exp_dist_anchor.size, active_size));
  }
  ...
  for (size_t i = 0; i + 1 < active_size; ++i) {
    int d = abs(clusters[active_region[i + 1]].genomic_pos -
                clusters[active_region[i]].genomic_pos);
    dist = static_cast<float>(genomicLengthToDistance(d));
    clusters[active_region[i]].dist_to_next = dist;
    noise_size += dist;
    // printf("%d d=%d ; %d %d ; %f\n", i, d,
    // clusters[active_region[i]].genomic_pos,
    // clusters[active_region[i+1]].genomic_pos, dist);
  }

  // we will use distance between neighbors as a measure of noise - it will be
  // used eg. to modify magnitude of points displacement during MC simulation
  noise_size /= active_region.size();
  noise_size *= smooth ? Settings::noiseCoefficientLevelSubanchor
                       : Settings::noiseCoefficientLevelAnchor;
  output(5, "noise size = %f   %d %d\n", noise_size, active_region.size(),
         active_size);

  // * initial structure
  // structure for a segment will typically consist of several tight loop
  // clusters. Number of these clusters depend on how anchors are connected by
  // loops. We use different initial structures for anchor and subanchor level,
  // based on a fact that loops clusters anchors are close to each other (r),
  // but distances between them may be large (R). Loop size is large as well
  // (R), so that loops span whole TD here R is approximate size of TD
  // (corresponding to current segment) and r distance between interacting
  // anchors
  // - on anchor level:
  //	  - identify loop clusters (maximal subsets of anchors that are
  //connected)
  //    - randomly position loop clusters 'origins' in distance R around the
  //    parent (segment) cluster position
  //    - for each loop cluster randomly position corresponding anchors in
  //    distance r from the loop cluster origin
  // - on subanchor level
  //    - (previously we interpolated subanchors between anchors beads, but this
  //    may not be the best idea,
  //      as now distances between anchors should be much smaller)
  //    - randomly position beads in distance R from ... something (eg. midpoint
  //    between neighboring anchor beads?)

  // TODO: how to best get values for R and r?
  // TODO: check if there are actually multiple loops clusters in segments
  // (there are many arcs, so it may always be just one)
  ...
  if (smooth) {
    if (Settings::useSubanchorHeatmap && !use_subanchor_heatmap)
      steps = Settings::subanchorEstimateDistancesSteps;
    else
      steps = Settings::simulationStepsLevelSubanchor;
  }

  // smooth ? Settings::simulationStepsLevelArcsSmooth :
  // Settings::simulationStepsLevelArcs; if (!smooth &&
  // Settings::useSubanchorHeatmap && !use_subanchor_heatmap) steps /= 5;
  ...
  for (int k = 0; k < steps; ++k) {
    ...
    // set random initial position (use initial_structure and add some noise,
    // but only to subanchors (ie. not-anchors))
    for (size_t i = 0; i < active_size; ++i) {
      clusters[active_region[i]].pos = initial_structure[i];
      if (!smooth || !clusters[active_region[i]].is_fixed)
        clusters[active_region[i]].pos += random_vector(
            smooth ? noise_size : noise_size_small, Settings::use2D);
    }
    ...
    // Energy trace logging after GPU arcs/smooth kernel
    if (Settings::energyTraceEnabled && sim_logger) {
      double domain_e = energy.domain_scale.totalEnergy();
      double loop_e = score;
      sim_logger->logEnergy(total_mc_steps, 0.0,
                            domain_e, loop_e, domain_e + loop_e, 0.0, 0.0);
    }

    output(3, "score = %lf, best = %lf\n", score, best_score);

    // if current structure is better than best so far than save current
    // structure as best
    if (score < best_score || best_score < 0.0) {
      best_structure.clear();
      for (size_t i = 0; i < active_size; ++i)
...
inline void LooperSolver::interpolateChildrenPosition(std::vector<int> &regions) {
  for (size_t i = 0; i < regions.size(); ++i) {
    interpolatePosition(clusters[regions[i]].children, regions, static_cast<int>(i));
  }
}

// interpolate points corresponding to 'regions'.
// we use this to calculate initial position for beads of subanchor level (ie.
// we interpolate them between anchor-level beads)
inline void LooperSolver::interpolatePosition(std::vector<int> &regions,
                                       std::vector<int> &clusters_template,
                                       int region_ind, bool noise) {
  int p1 = region_ind - 1;
  int p2 = region_ind;
  size_t n = regions.size();
  float dst = 1.0f / n;
  float st = 0.5f + dst / 2.0f;
  ...
  // calc noise to add - use fraction of avg distance between consecutive
  // points.
  float avg_disp = 0.0f;
  if (noise) {
    if (clusters_template.size() > 1) {
      for (size_t i = 1; i < clusters_template.size(); ++i)
        avg_disp += (clusters[clusters_template[i]].pos -
                     clusters[clusters_template[i - 1]].pos)
                        .length();
      avg_disp /= (clusters_template.size() - 1); // take average...
      avg_disp *= 0.25f;                          // ... and the fraction of it
    } else
      avg_disp = 0.0f; // if there is only one point do not add any noise (TODO:
                       // a global noise magnitude)
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if (!clusters[regions[i]].is_fixed) {
      clusters[regions[i]].pos =
          interpolate(clusters[clusters_template[p1]].pos,
                      clusters[clusters_template[p2]].pos, st);
      if (noise)
        clusters[regions[i]].pos += random_vector(avg_disp, Settings::use2D);
    }
...
inline void LooperSolver::interpolateChildrenPositionSpline(std::vector<int> &regions,
                                                     bool use_genomic_dist) {
  ...
  for (size_t i = 2; i < 4; ++i)
    pts[i] = clusters[regions[i - 2]].pos;

  // calc two last points
  vector3 end_pt = mirrorPoint(clusters[regions[reg_cnt - 1]].pos,
                               clusters[regions[reg_cnt - 2]].pos);
  ...
  for (size_t i = 0; i < reg_cnt; ++i) {
    ...
    int switch_controls =
        -1; // index of child after which we should switch control points by one
    ...
    if (use_genomic_dist) {
      ...
      // * first (old) version:
      // find start and end location of region (segment + half of flanking
      // between-segments regions) for every children get position relative to
      // the region and transform it to the knot the problem is that if right
      // flanking region is much bigger than it will bias all the regions to the
      // left (beacuse the relative positions will get smaller)

      // * second version:
      // as before, but find positions relative to the corresponding halves - if
      // the children is on the left of the segment center than find position
      // relative to the left flanking region and left half of the segment. This
      // way if the right flanking region is geting bigger and bigger it won't
      // influence the anchors on the left side
      ...
      // we need also to consider flanking between-segments space (we use center
      // of this space)
      int left_flank = i == 0 ? start_loc : clusters[regions[i - 1]].end;

      size_t right_flank =
          i == regions.size() - 1 ? end_loc : clusters[regions[i + 1]].start;

      // calculate left and right position including flanking regions
      start_loc = (left_flank + start_loc) / 2;
      end_loc = static_cast<int>((right_flank + end_loc) / 2);
      ...
      // for every child calculate its location relative to (start_loc, end_loc)
      // and transform it to a knot [0.5->0.9->1.0->0.1->0.5]
      for (size_t j = 0; j < n; ++j) {
        int pos = clusters[clusters[regions[i]].children[j]].genomic_pos;
        float p = 1.0f;
        if (pos < center_loc) {
          p = (pos - start_loc) / left_length; // location proportion [0, 1]
          p = 0.5f + p * 0.5f;                 // transform to [0.5, 1]
        } else {
          p = (pos - center_loc) / right_length;
          p = p * 0.5f; // transform to [0, 0.5]
          if (switch_controls == -1)
            switch_controls = static_cast<int>(j); // this was the first child to cross 1.0
        }
        knots.push_back(p);
      }
    } else {
    ...
    for (size_t j = 0; j < n; ++j) {
      if ((int)j == switch_controls || (switch_controls == -1 && j == n - 1)) {
        ...
        if (i + 2 == regions.size())
          pts[3] = end_pt;
        else if (i + 2 == regions.size() + 1)
          pts[3] = end_pt2;
        else
          pts[3] = clusters[regions[i + 2]].pos;
      }
...
inline double LooperSolver::MonteCarloArcs(float step_size) {
  ...
  // the idea to keep the true score without recalculating it every time is to
  // calculate total score once, on the beginning. during the simulation we
  // calculate local score before and after a move new score is then equal to
  // (old total score) - (local score before move) + (local score after move)

  // Rebuild loop constraint map for current active region
  rebuildActiveLoopMap();

  // calculate total score (arc distances + loop energy + boundary)
  score_curr = calcScoreDistancesActiveRegion() + calcScoreLoopEnergy();
  if (spherical_container) {
    for (int k = 0; k < size; k++)
      score_curr += spherical_container->boundaryPenalty(clusters[active_region[k]].pos);
  }

  score_prev = score_curr;
  milestone_score = score_curr;

  output(4, "size = %d; initial score = %lf (loop constraints = %d), step size = %f\n",
         size, score_curr, (int)active_loop_map.size(), step_size);
  ...
  while (true) {
    ...
    if (clusters[ind].is_fixed)
      error("cluster fixed during arcs!\n");

    local_score_prev = calcScoreDistancesActiveRegion(p) + calcScoreLoopEnergy(p);
    if (spherical_container)
      local_score_prev += spherical_container->boundaryPenalty(clusters[ind].pos);

    tmp = random_vector(step_size, Settings::use2D);
    clusters[ind].pos += tmp;

    local_score_curr = calcScoreDistancesActiveRegion(p) + calcScoreLoopEnergy(p);
    if (spherical_container)
      local_score_curr += spherical_container->boundaryPenalty(clusters[ind].pos);
    ...
    if (!ok) {
      ...
      // score_prev, score_curr/score_prev, T, tp, ok);
    }
    ...
    if (i % Settings::MCstopConditionSteps == 0) {
      output(7,
             "milestone: score = %lf, last = %lf (%lf), T=%lf successes = %d, "
             "last = %d\n",
             score_curr, milestone_score, score_curr / milestone_score, T,
             success, milestone_success);

      // stop if the improvement since the last milestone is less than
      // threshold, or if the score is too small (may happen when heatmap is
      // small and points are perfectly arranged)
      if ((score_curr >
               Settings::MCstopConditionImprovement * milestone_score &&
           milestone_success < Settings::MCstopConditionMinSuccesses) ||
  ...
  // Log final acceptance ratio for this phase
  if (sim_logger)
    sim_logger->logAcceptanceRatio("arcs_anchor", i, success);

  total_mc_steps += i;
  return score_curr;
}

inline double LooperSolver::MonteCarloArcsSmooth(float step_size,
                                          bool use_subanchor_heatmap) {
  ...
  double curr_score_structure = 0.0, prev_score_structure = 0.0,
         local_score_prev_structure = 0.0, local_score_curr_structure = 0.0;
  double curr_score_orientation = 0.0, prev_score_orientation = 0.0,
         local_score_prev_orientation = 0.0, local_score_curr_orientation = 0.0;
  double curr_score_heat = 0.0, prev_score_heat = 0.0,
         local_score_prev_heat = 0.0, local_score_curr_heat = 0.0;
  double milestone_score; // score measured every N steps, used to see whether
                          // the solution significantly improves
  int success = 0;        // overall number of successes
  int milestone_success =
  ...
  // when we move an anchor we may need to recalculate the orientation. then, if
  // we reject the move, we need to restore it prev_orientation will store old
  // orientation, and orn_index denotes the index of anchor for which the
  // orientation has changed
  int orn_index = -1; // -1 means that there was no change of orientation. >=0
                      // values denotes an 'anchor_orientation' index
  vector3 prev_orientation;
  int anchor_index; // tmp variable

  // mark anchors and their neighbors (used to see whether orientation score
  // changed) for every bead in active_region we assign a coded value coding: 2
  // - right anchor neighbor, 1 - left neighbor for anchors we use 3+index, ie.
  // 3 for first anchor, 4 for second etc (used to quickly get orientation, this
  // index refers to 'anchor_orientation') negative values: if cluster 'i' is
  // anchor, then cell i-2 keeps negative index of anchor neighboring from left,
  // and i+2 - from right this allows us to immediately see whether particular
  // cluster is anchor, and what are neighbors (required to calculate score)
  // note: for this to work we need at least 4 subanchor clusters between
  // anchors sample result: [3 2 -6 0 0 1 4 2 -12 0 -6 1 5 2 -18 0 -12 1 6 2 -24
  // 0 -18 1 7] so active_region[5] is a left neighbor of the second anchor
  // (with cluster-index: 4-3=1), neighboring anchors are in 0 and 12 anchors
  // are denoted by 3,4,5,6,7.
  vector<vector3>
      anchor_orientation; // store anchor orientation (we need to update this
                          // after anchors or neighbors moving)
  ...
  if (Settings::useCTCFMotifOrientation) {
    ...
    for (int i = 0; i < size; i++) {
      if (clusters[active_region[i]].is_fixed) {
        ...
        anchor_orientation.push_back(
            calcOrientation(i)); // calculate and save orientation
  ...
  if (spherical_container) {
    for (int k = 0; k < size; k++)
      curr_score_structure += spherical_container->boundaryPenalty(clusters[active_region[k]].pos);
  }
  if (Settings::useCTCFMotifOrientation)
    curr_score_orientation = calcScoreOrientation(anchor_orientation);
  if (use_subanchor_heatmap && Settings::useSubanchorHeatmap)
    curr_score_heat = calcScoreSubanchorHeatmap();
  ...
  if (!silent)
    output(4, "size = %d; initial score = %lf (%lf %lf %lf), step size=%f\n",
           size, score_curr, curr_score_structure, curr_score_orientation,
           curr_score_heat, step_size);
  ...
  while (true) {
    ...
    orn_index = -1; // -1 means that there was no change of orientation. >=0
                    // values denotes an 'anchor_orientation' index
    if (Settings::useCTCFMotifOrientation) {
      if (cluster_type[p] > 0) {
        ...
        if (cluster_type[p] == 1)
          anchor_index = p + 1;
        else if (cluster_type[p] == 2)
          anchor_index = p - 1;

        // now extract the corresponding index in anchor_orientation
        orn_index =
            cluster_type[anchor_index] - 3; // index is coded as (index+3)

        prev_orientation =
            anchor_orientation[orn_index]; // store current orientation in case
                                           // we reject the move
        // anchor_orientation[orn_index] = calcOrientation(anchor_index);

        local_score_prev_orientation =
            calcScoreOrientation(anchor_orientation, orn_index);
      }
    }

    local_score_prev_structure = calcScoreStructureSmooth(p, true, true);
    if (spherical_container)
      local_score_prev_structure += spherical_container->boundaryPenalty(clusters[ind].pos);
    if (use_subanchor_heatmap && Settings::useSubanchorHeatmap)
      local_score_prev_heat = calcScoreSubanchorHeatmap(p);
    ...
    if (spherical_container)
      local_score_curr_structure += spherical_container->boundaryPenalty(clusters[ind].pos);
    if (orn_index != -1) {
      anchor_orientation[orn_index] = calcOrientation(anchor_index);
      local_score_curr_orientation =
          calcScoreOrientation(anchor_orientation, orn_index);
      assert(local_score_curr_orientation >= 0.0f);
      curr_score_orientation =
          curr_score_orientation +
    ...
    if (use_subanchor_heatmap && Settings::useSubanchorHeatmap) {
      ...
      curr_score_heat = curr_score_heat +
                        2.0 * (local_score_curr_heat - local_score_prev_heat);
    }

    curr_score_structure = curr_score_structure - local_score_prev_structure +
                           local_score_curr_structure;
    // curr_score_structure = calcScoreStructureSmooth(true, true);

    score_curr =
        curr_score_structure + curr_score_orientation + curr_score_heat;

    ok = score_curr < score_prev;

    if (!ok && T > 0.0) {
      tp = Settings::tempJumpScaleSmooth *
           exp(-Settings::tempJumpCoefSmooth * (score_curr / score_prev) / T);
      ok = withChance(static_cast<float>(tp));
      // printf("%f %f %f %f   %d\n", score_curr, score_prev, T, tp, ok);
    }
    ...
    if (ok) {
      ...
      if (orn_index >= 0) {
        ...
        anchor_orientation[orn_index] =
            prev_orientation; // ... and orientation (if needed)
      }
    }
    ...
    if (Settings::energyTraceEnabled && sim_logger &&
      ...
      double accept_ratio = (i > 0) ? (double)success / i : 0.0;
      sim_logger->logEnergy(total_mc_steps + i, T,
                            curr_score_structure, curr_score_orientation,
                            score_curr, accept_ratio, 0.0);
    }
    ...
    if (i % Settings::MCstopConditionStepsSmooth == 0) {
      ...
      if (!silent) {
        output(7,
               "milestone: score = %lf (%lf %lf %lf), last = %lf (%lf), T=%lf "
               "successes = %d, last = %d\n",
               score_curr, curr_score_structure, curr_score_orientation,
               curr_score_heat, milestone_score, score_curr / milestone_score,
               T, success, milestone_success);
      }

      if ((score_curr >
               Settings::MCstopConditionImprovementSmooth * milestone_score &&
           milestone_success < Settings::MCstopConditionMinSuccessesSmooth) ||
          score_curr < 1e-6)
        break;
  ...
  // Log final acceptance ratio for this phase
  if (sim_logger)
    sim_logger->logAcceptanceRatio("arcs_smooth", i, success);
...
Heatmap
inline LooperSolver::calculateRegionsDistances(const std::vector<int> clusters_ind) {
  int n = static_cast<int>(clusters_ind.size());
  Heatmap h(n);
  for (int i = 0; i < n; ++i) {
    for (int j = i + 1; j < n; ++j) {
      h.v[i][j] =
          (clusters[clusters_ind[i]].pos - clusters[clusters_ind[j]].pos)
              .length();
      h.v[j][i] = h.v[i][j];
    }
  }
...
inline string LooperSolver::activeRegionIndexToChr(int ind) {
  for (string chr : chrs) {
    if (ind < active_region_to_chr[chr])
      return chr;
  }
  return "";
}

inline std::set<int> LooperSolver::getCurrentHeatmapBreaks() {
  std::set<int> r;
  int curr_ind = 0;
  string chr;
  for (size_t j = 0; j < chrs.size(); j++) {
...
inline void LooperSolver::setDebuggingOptions(int chr_number_limit, int length_limit) {
...
// first find the tangent assuming orientation is R, and then, if it is L, just
// take the negative
inline vector3 LooperSolver::calcOrientation(int cind) {
  vector3 orn;
  if (cind == 0)
    orn = clusters[active_region[cind + 1]].pos -
          clusters[active_region[cind]].pos;
  else if (cind == (int)active_region.size() - 1)
    orn = clusters[active_region[cind]].pos -
          clusters[active_region[cind - 1]].pos;
  else
    orn = clusters[active_region[cind + 1]].pos -
          clusters[active_region[cind - 1]].pos;

  if (clusters[active_region[cind]].orientation == 'L')
    orn *= -1.0;
...
inline void LooperSolver::showCoordinatesRange(int level) {
  ...
  for (string chr : chrs) {
    for (size_t i = 0; i < hc.chr[chr].points.size(); ++i) {
      for (size_t j = 0; j < dim; ++j) {
        if (hc.chr[chr].points[i][static_cast<unsigned int>(j)] < min[static_cast<unsigned int>(j)])
          min[static_cast<unsigned int>(j)] = hc.chr[chr].points[i][static_cast<unsigned int>(j)];
        if (hc.chr[chr].points[i][static_cast<unsigned int>(j)] > max[static_cast<unsigned int>(j)])
          max[static_cast<unsigned int>(j)] = hc.chr[chr].points[i][static_cast<unsigned int>(j)];
      }
    }
  }
...
inline void LooperSolver::showCoordinatesRange(bool use_active_region) {
  ...
  if (use_active_region) {
    ...
    for (size_t i = 0; i < active_region.size(); ++i) {
      for (size_t j = 0; j < dim; ++j) {
        if (clusters[active_region[i]].pos[static_cast<unsigned int>(j)] < min[static_cast<unsigned int>(j)])
          min[static_cast<unsigned int>(j)] = clusters[active_region[i]].pos[static_cast<unsigned int>(j)];
        if (clusters[active_region[i]].pos[static_cast<unsigned int>(j)] > max[static_cast<unsigned int>(j)])
          max[static_cast<unsigned int>(j)] = clusters[active_region[i]].pos[static_cast<unsigned int>(j)];
      }
    }
...
inline void LooperSolver::calcAnchorsStats() {
  setLevel(LVL_SEGMENT);
  std::map<std::string, std::vector<int>> breaks;
  ...
  for (string chr : chrs) {
    ...
    for (size_t i = 0; i < current_level[chr].size(); i++) {
      ...
      ib_in_segments_chr.push_back(static_cast<int>(clusters[curr].children.size()));
      ib_in_segments.push_back(static_cast<int>(clusters[curr].children.size()));

      if (i + 1 < current_level[chr].size()) {
        breaks[chr].push_back((clusters[curr].end + clusters[next].start) / 2);
        inter_seg_length[chr].push_back(clusters[next].start -
                                        clusters[curr].end);
      }
    }
    breaks[chr].push_back(1000000000);

    // saveToCSV(ftext("/d/tmp/motifs_gb/segment_len_%s.txt", chr.c_str()),
    // seg_length_chr);

    // saveToCSV(ftext("/d/tmp/motifs_gb/ib_in_segments_%s.txt", chr.c_str()),
    // ib_in_segments_chr);
  }
  ...
  for (string chr : chrs) {
    ...
    setLevel(LVL_INTERACTION_BLOCK);
    int size = static_cast<int>(current_level[chr].size());
    printf("%s, size=%d\n", chr.c_str(), size);
    ...
    for (int i = 0; i < size; ++i) {
      ...
      if (i + 1 < size)
        ib_dist.push_back(clusters[ind + 1].start - clusters[ind].end);
      //			printf("%s %d %d %d %d\n", chr.c_str(),
      //clusters[ind].start, clusters[ind].end, clusters[ind].base_start,
      //clusters[ind].base_end);
    }

    // saveToCSV(ftext("/d/tmp/motifs_gb/ib_len_%s.txt", chr.c_str()),
    // ib_length_chr);
  }
  ...
  for (size_t i = 0; i < arcs_singletons.size(); ++i) {
    ...
    while (!feof(f)) {
      ...
      if (chrs_set.find(chr1) == chrs_set.end())
        continue;
      if (chrs_set.find(chr2) == chrs_set.end())
        continue;

      // find indices of start and end of the arc
      int segm_ind_a = -1, segm_ind_b = -1; // cluster-index of segment
      for (size_t j = 0; j < current_level[chr1].size(); ++j) {
        if (clusters[current_level[chr1][j]].contains(sta))
          segm_ind_a = current_level[chr1][j];
        if (clusters[current_level[chr1][j]].contains(enda))
          segm_ind_b = current_level[chr1][j];
      }
      ...
      int anch_ind_a = -1,
          anch_ind_b = -1; // cluster-index of anchor within current segment
      bool inside_anchor_a,
          inside_anchor_b; // is sigleton starting within anchor, or
                           // between-anchor region?
      for (size_t j = 0; j < clusters[segm_ind_a].children.size(); ++j) {
        ...
        if (anch_ind_a != -1 && anch_ind_b != -1)
          break;
      }

      //			printf("anch ind = %d %d (%d %d)\n", anch_ind_a,
      //anch_ind_b, inside_anchor_a, inside_anchor_b); 			if (anch_ind_a != -1) {
      //				if (anch_ind_a > 0)
      //clusters[anch_ind_a-1].print(); 				clusters[anch_ind_a].print();
      //			}
      //			if (anch_ind_b != -1) {
      //				if (anch_ind_b > 0)
      //clusters[anch_ind_b-1].print(); 				clusters[anch_ind_b].print();
      //			}
      ...
      if (anch_ind_a == anch_ind_b || (abs(anch_ind_a - anch_ind_b) == 1 &&
                                       inside_anchor_a != inside_anchor_b))
        intra_loops++;
      else
        inter_loops++;
    }
...
Heatmap
inline LooperSolver::calcTrueDistancesHeatmapForRegion(std::vector<int> &regions) {
  Heatmap h;
  int i, j;
...
// use heatmap with avg distance between beads and subanchor level singleton
// heatmap distances are based on avg distances, but we also consider subanchor
// heatmap assumes that 'heatmap_subanchor' is created result is saved to
// 'heatmap_dist_subanchor'
inline void LooperSolver::createExpectedDistSubanchorHeatmap(
    const Heatmap &subanchor_avg_dist) {
  if (subanchor_avg_dist.size != heatmap_subanchor.size)
    error("subanchor heatmaps size mismatch!");

  int n = static_cast<int>(subanchor_avg_dist.size);
  ...
  printf("avg = %f\n", avg_val);
  if (avg_val < 1e-6)
    return; // heatmap is empty, we can do nothing

  float s;
  for (int i = 0; i < n; ++i) {
    for (int j = i + 1; j < n; ++j) {
      s = (heatmap_subanchor.v[i][j] / avg_val) *
          Settings::subanchorHeatmapInfluence;
      if (s > 1.0f)
        s = 1.0f;
...
inline void LooperSolver::calcActiveAnchorsNeighbors() {
  int ai, arc;
  int other_end;
  ...
  for (size_t i = 0; i < active_region.size(); ++i) {
    ...
    if (clusters[ai].arcs.size() || clusters[ai].is_fixed) {
      cluster_to_active_anchor_index[ai] = static_cast<int>(anchors.size());
      anchors.push_back(static_cast<int>(i));
    }
  }

  // for every anchor iterate through all outcoming arcs
  // if they end in another anchor (and they should) then add them (ie. their
  // active_region-anchor index) to result
  float w;
  for (size_t i = 0; i < anchors.size(); ++i) {
    ...
    for (size_t j = 0; j < clusters[ai].arcs.size(); ++j) {
      ...
      active_anchors_neighbors[static_cast<int>(i)].push_back(
          cluster_to_active_anchor_index[other_end]);
      w = static_cast<float>(sqrt(arcs.arcs[current_chr][arc].score));
      active_anchors_neighbors_weight[static_cast<int>(i)].push_back(w);
    }
  }
}
...
inline void LooperSolver::calcAnchorExpectedDistancesHeatmap() {
  int ai = 0, other_end = 0, arc = 0, freq;
  double exp_dist = 0.0;

  // init with -1 values
  heatmap_exp_dist_anchor.init(static_cast<int>(active_region.size()));
  ...
  for (size_t i = 0; i < active_region.size(); ++i) {
    cluster_to_active_index[active_region[i]] = static_cast<int>(i);
    active_to_cluster_index[static_cast<int>(i)] = active_region[i];
  }
    ...
    for (size_t j = 0; j < clusters[ai].arcs.size(); ++j) {
      ...
      if (other_end == -1) {
        printf("no other end (%d %d, %d %d)!\n", arc, ai,
               arcs.arcs[current_chr][arc].start,
               arcs.arcs[current_chr][arc].end);
        exit(0);
        continue;
      }

      if (other_end < ai)
        continue; // only consider forward loops (to count each arc only once)

      // note: 'arc' is chromosome specific, but we know that we work on
      // 'current_chr' right now
      ...
      heatmap_exp_dist_anchor.v[a][b] = static_cast<float>(exp_dist);
      heatmap_exp_dist_anchor.v[b][a] = static_cast<float>(exp_dist);
    }
  }
  ...
  if (Settings::useAnchorHeatmap) {
    ...
    if (heatmap_exp_dist_anchor.size != heatmap_anchor.size)
      error("anchor heatmaps size mismatch!");
    ...
    if (max_val > 1e-6) {
      int n = static_cast<int>(heatmap_anchor.size);
      float s;
      for (int i = 0; i < n; ++i) {
        for (int j = i + 1; j < n; ++j) {
          if (heatmap_exp_dist_anchor.v[i][j] < 0.0f)
          ...
          s = (heatmap_anchor.v[i][j] / max_val) *
              Settings::anchorHeatmapInfluence;
          if (s > 1.0f)
            s = 1.0f;
...
inline int LooperSolver::getGenomicLengthExcludingCentromere(string chr, int st,
                                                      int end) {
  if (centromeres.regions.size() == 0)
    return end - st + 1; // no info about centromeres
  for (size_t i = 0; i < centromeres.regions.size(); ++i) {
    if (centromeres.regions[i].chr == chr) {
      ...
      if (cs > end || ce < st)
        return end - st + 1; //  p q  A  B
      if (cs < st) {
        if (ce < end)
          return end - ce + 1; // p A  q  B
        return 0;              // p A    B  q
      }
      if (ce < end)
        return end - st - (ce - cs + 1) + 1; //   A   p  q  B
...
inline void LooperSolver::printStructuresForGenomicPosition(string chr, int pos) {
  int curr_lev, clust;
...
inline int LooperSolver::findClusterForGenomicPosition(string chr, int pos) {
  for (size_t i = 0; i < current_level[chr].size(); ++i) {
    int c = current_level[chr][i];
    if (clusters[c].contains(pos))
      return c;
  }
  return -1;
}

inline int LooperSolver::findCurrentLevelForGenomicPosition(string chr, int pos) {
  for (size_t i = 0; i < current_level[chr].size(); ++i) {
    int c = current_level[chr][i];
    if (clusters[c].contains(pos))
...
inline void LooperSolver::output(int level, const char *format, ...) {
...
inline void LooperSolver::inputArbitraryHeatmap(Heatmap h) {
  clusters.clear();
  active_region.clear();
...
inline Heatmap LooperSolver::createIFHeatmapFromDistances(const Heatmap &heat) {
  Heatmap h(static_cast<int>(heat.size));
  for (size_t i = 0; i < heat.size; ++i) {
    for (size_t j = 0; j < heat.size; ++j) {
...
inline void LooperSolver::getChromosomeHeatmapBoundary(int p, int &start, int &end) {
  if (heatmap_chromosome_boundaries.size() == 0)
    return;
  if (p < 0 ||
      p > heatmap_chromosome_boundaries[heatmap_chromosome_boundaries.size() -
                                        1])
    return;

  for (size_t i = 0; i + 1 < heatmap_chromosome_boundaries.size(); ++i) {
    if (heatmap_chromosome_boundaries[i] <= p &&
        p < heatmap_chromosome_boundaries[i + 1]) {
...
inline void LooperSolver::reset() {
  for (size_t i = 0; i < clusters.size(); i++) {
...
// remove the subanchor beads (remove the corresponding clusters, clean
// cluster::children etc)
inline void LooperSolver::removeSubanchorBeads() {
  // we use the property that the subanchor beads are placed on the very end of
  // the 'clusters', and that for a given chromosome the root is always the last
  // non-subanchor entry. Thus, to locate all subanchors we only need to find
  // the index of the last root. All the following entries are subanchor beads.
  ...
  for (i = 0; i < chrs.size(); ++i) {
    if (chr_root[chrs[i]] > first_subanchor_bead)
      first_subanchor_bead = chr_root[chrs[i]];
  }
  if (first_subanchor_bead == -1)
    error("could not find the subanchor beads starting index");
  // printf("root = %d\n", first_subanchor_bead);
  first_subanchor_bead++; // it was pointing to the last root, now to the first
                          // subanchor

  // while (first_subanchor_bead<clusters.size() &&
  //		clusters[first_subanchor_bead].start !=
  //clusters[first_subanchor_bead].end) first_subanchor_bead++;
  ...
  clusters.erase(clusters.begin() + first_subanchor_bead,
                 clusters.end()); // remove the subanchor beads

  // remove their indicies from 'children' list
  for (i = 0; i < clusters.size(); ++i) {
    if (clusters[i].children.size() == 0)
      continue;

    for (int j = static_cast<int>(clusters[i].children.size()) - 1; j >= 0; --j) {
      if (clusters[i].children[j] >= first_subanchor_bead)
        clusters[i].children.erase(clusters[i].children.begin() + j);
    }
  }
}

inline vector3 LooperSolver::densityCoordToStructure(vector3 coord) {
...
inline vector3 LooperSolver::structureCoordToDensity(vector3 pos) {
  pos = (pos - density.center) * Settings::densityScale;
  pos.x /= 3.0f;
  pos += density.center - density.origin;
  return vector3(static_cast<float>((int)pos.x), static_cast<float>((int)pos.y), static_cast<float>((int)pos.z));
}

// ─── RMSD Convergence Tracking (F5.3) ──────────────────────────────────────

inline void LooperSolver::logRMSDCheckpoint(const std::string &label) {
  std::vector<vector3> positions;
  // Store positions of all clusters across all chromosomes at the current level
  for (const std::string &chr : chrs) {
    if (current_level.find(chr) == current_level.end())
      continue;
    const std::vector<int> &lvl = current_level[chr];
    for (size_t j = 0; j < lvl.size(); ++j) {
      int idx = lvl[j];
      if (idx >= 0 && idx < (int)clusters.size())
        positions.push_back(clusters[idx].pos);
    }
  }
  rmsd_checkpoints[label] = positions;
  output(3, "[RMSD] Checkpoint '%s' saved (%d positions)\n",
         label.c_str(), (int)positions.size());
}

inline double LooperSolver::computeRMSDvsCheckpoint(const std::string &label) const {
  auto it = rmsd_checkpoints.find(label);
  if (it == rmsd_checkpoints.end())
    return -1.0;

  const std::vector<vector3> &ref = it->second;

  // Gather current positions at the same level
  std::vector<vector3> current_positions;
  for (const std::string &chr : chrs) {
    auto cl_it = current_level.find(chr);
    if (cl_it == current_level.end())
      continue;
    const std::vector<int> &lvl = cl_it->second;
    for (size_t j = 0; j < lvl.size(); ++j) {
      int idx = lvl[j];
      if (idx >= 0 && idx < (int)clusters.size())
        current_positions.push_back(clusters[idx].pos);
    }
  }

  int n = std::min((int)ref.size(), (int)current_positions.size());
  if (n < 2)
    return 0.0;

  // Compute RMSD manually (sum of squared distances)
  double sum_sq = 0.0;
  for (int i = 0; i < n; ++i) {
    double dx = (double)current_positions[i].x - (double)ref[i].x;
    double dy = (double)current_positions[i].y - (double)ref[i].y;
    double dz = (double)current_positions[i].z - (double)ref[i].z;
    sum_sq += dx * dx + dy * dy + dz * dz;
  }
  double rmsd_val = sqrt(sum_sq / n);

  return rmsd_val;
}

inline void LooperSolver::writeRMSDConvergenceCSV(const std::string &suffix) const {
  if (rmsd_convergence_log.empty())
    return;

  std::string path = ftext("%srmsd_convergence_%s.csv",
                            output_dir.c_str(), suffix.c_str());
  FILE *f = fopen(path.c_str(), "w");
  if (!f) {
    printf("[RMSD] Failed to open %s for writing\n", path.c_str());
    return;
  }

  fprintf(f, "phase,rmsd\n");
  for (const auto &entry : rmsd_convergence_log) {
    fprintf(f, "%s,%.6f\n", entry.first.c_str(), entry.second);
  }
  fclose(f);
  printf("[RMSD] Convergence data written to %s\n", path.c_str());

  // Generate RMSD convergence SVG chart
  std::string svg_path = ftext("%srmsd_convergence_%s.svg",
                                output_dir.c_str(), suffix.c_str());
  if (SvgChartGenerator::rmsdConvergenceChart(path, svg_path)) {
    printf("[SVG] Chart written: %s\n", svg_path.c_str());
  }

  // Also log to SimulationLogger if available
  if (sim_logger) {
    const_cast<SimulationLogger *>(sim_logger)->logMilestone(
        ftext("RMSD convergence written: %s", path.c_str()));
  }
}
