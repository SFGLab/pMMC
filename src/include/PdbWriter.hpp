...
inline bool PdbWriter::write(HierarchicalChromosome &hc, const std::string &filename,
  ...
  // Annotation type encoding in B-factor column
  fprintf(f, "REMARK 999 BFACTOR_ANNOTATION NONE=0.00 CTCF=10.00 ENHANCER=20.00 PROMOTER=30.00 EPIGENETIC=40.00\n");
  ...
  for (size_t i = 0; i < beads.size(); ++i) {
    ...
    // B-factor encodes annotation type for visualization in Chimera/PyMOL:
    //   0.00 = NONE, 10.00 = CTCF, 20.00 = ENHANCER, 30.00 = PROMOTER, 40.00 = EPIGENETIC
    // Occupancy encodes hierarchy level (as float)
    float bfactor = (float)cl.annotation_type * 10.0f;
    fprintf(f,
            "ATOM  %5d  CA  GLY %c%4d    %8.3f%8.3f%8.3f%6.2f%6.2f          C \n",
            serial, chainId, resSeq, nx, ny, nz,
            (float)cl.level, bfactor);
...
inline bool PdbWriter::writeFlat(const std::vector<vector3> &positions,
                          const std::string &chrName,
                          const std::string &filename) {
  FILE *f = fopen(filename.c_str(), "w");
  if (!f) {
