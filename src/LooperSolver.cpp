...
void LooperSolver::setContactData(std::vector<string> chrs_list,
...
std::vector<int> LooperSolver::findSplit(std::vector<int> gaps, int exp_size,
                                         string chr) {
...
void LooperSolver::createSingletonSubanchorHeatmap(
...
float LooperSolver::getHeatmapAvgBetweenNeighboringRegions(Heatmap &heat,
                                                           int level) {
  int i, j, k;
  int n;
...
void LooperSolver::loadLoopConstraints(const std::string &filepath) {
...
void LooperSolver::rebuildActiveLoopMap() {
...
double LooperSolver::calcScoreLoopEnergy() const {
  double sc = 0.0;
...
double LooperSolver::calcScoreLoopEnergy(int cluster_moved) const {
  double sc = 0.0;
  for (const auto &entry : active_loop_map) {
    int ai = entry.first.first;
    int aj = entry.first.second;
...
double LooperSolver::calcScoreStructureSmooth(int cluster_moved, bool lengths,
                                              bool angles) {
  double diff, ang, dtn;
  vector3 v, v2;
...
// 'cluster' belongs to
void LooperSolver::reconstructClusterArcsDistances(int cluster, int cluster_ind,
                                                   bool smooth,
...
void LooperSolver::interpolatePosition(std::vector<int> &regions,
...
void LooperSolver::interpolateChildrenPositionSpline(std::vector<int> &regions,
...
double LooperSolver::MonteCarloArcs(float step_size) {
  ...
  // calculate total score (arc distances + loop energy)
...
double LooperSolver::MonteCarloArcsSmooth(float step_size,
                                          bool use_subanchor_heatmap) {
...
Heatmap
LooperSolver::calculateRegionsDistances(const std::vector<int> clusters_ind) {
...
Heatmap
LooperSolver::calcTrueDistancesHeatmapForRegion(std::vector<int> &regions) {
  Heatmap h;
  int i, j;
...
void LooperSolver::createExpectedDistSubanchorHeatmap(
...
int LooperSolver::getGenomicLengthExcludingCentromere(string chr, int st,
                                                      int end) {
