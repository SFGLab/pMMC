...
bool MergedFileIO::writeMerged(const std::string& bed_path,
...
bool MergedFileIO::readMerged(const std::string& path,
                               std::vector<MergedLoop>& loops,
                               std::vector<MergedAnchor>& anchors) {
    loops.clear();
    anchors.clear();
...
void MergedFileIO::filterByChromosomes(std::vector<MergedLoop>& loops,
                                        std::vector<MergedAnchor>& anchors,
...
bool MergedFileIO::writeBed(const std::vector<MergedAnchor>& anchors,
...
bool MergedFileIO::writeBedpe(const std::vector<MergedLoop>& loops,
                               const std::string& path) {
    FILE* f = fopen(path.c_str(), "w");
    if (!f) {
